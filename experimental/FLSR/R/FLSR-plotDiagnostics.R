...
plotResidAll<-function(resid,obs,prd,hat,indVar,indVar.,Xttl="X",Yttl="Y",mttl1="Functional Form")
  	{
    ...
    plotFunc(          obs,prd,indVar,indVar.,xttl=Xttl,              yttl=Yttl,              mttl=mttl1,                  splt=c(1,1,2,3),more=TRUE)
    plotResidYr( resid,                       xttl="Year",            yttl='Residuals',       mttl="",                     splt=c(2,1,2,3),more=TRUE)
    plotResidAR1(resid,                       xttl=expression(Residuals[t]),  yttl=expression(Residuals[t+1]),mttl='AR(1) Residuals',      splt=c(1,2,2,3),more=TRUE)
...
setMethod("plot", signature(x="FLSR", y="missing"),function(x,y,...){
   if (length(model(x))==0) x<-lowess(x)
   ...
   indVar.<-apply(indVar,c(1,3:6),max)
   dmns   <-dimnames(indVar.)
   dmns$year<-1:(dim(rec(nsher))[2])
   indVar.<-sweep(FLQuant(seq(0,1,length.out=dim(rec(nsher))[2]),dimnames=dmns),c(1,3:6),indVar.,"*")
   prd    <-predict(x,ssb=indVar.)

   plotResidAll(resid,obs,prd,hat,indVar,indVar.,Yttl="Recruits",Xttl="SSB",mttl1="Stock Recruit Relationship")})
