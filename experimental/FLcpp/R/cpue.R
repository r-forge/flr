...
setValidity("cpue", validcpue)
remove(validcpue)   

cpue <- function(name=character(0), desc=character(0), distribution=character(0),
  ...
  sdimnames[1] <- 'all'
	...
	new <- new("cpue", name = name, desc = desc, distribution = distribution,
        type=type,
        index = index,
        ...
        catch.n = FLQuant(dimnames=dimnames), 
        catch.wt = FLQuant(dimnames=dimnames),
        effort = FLQuant(dimnames=sdimnames),
        catch   = FLQuant(dimnames=sdimnames),
...
setAs('FLStock', 'cpue',
    ...
    dmns$year<-1
		res<-cpue(        index       =from@catch.n,
                      ...
                      catch       =apply(from@catch[  ac(from@range["min"]:from@range["max"])],2:6,mean),
...
setMethod("logl", signature(object="cpue"),
   function(object, hat, ...) {
...
setMethod("show", signature(object="cpue"),
  function(object){
    ...
    # FLArray slots
		...
		for (s in qnames)
        if (all(!is.na(slot(object,s))))
				  print(slot(object,s))
})	

setMethod("summary", signature(object="cpue"),
	function(object, ...){
