...
# Last Change: 02 Apr 2010 10:58
# $Id$

# plot {{{
setMethod("plot", signature(x="FLBRP", y="missing"),
  function(x, y, type=c("all", "yield.harvest", "ssb.harvest", "rec.ssb",
    "yield.ssb", "profit.harvest", "profit.ssb", ""),cols=c(1, 2, 8, 5, 6, 10, 12),
    obs=FALSE,refpts=TRUE,ts="missing",...)
  {
  
    lim.h <- c(0, ifelse(all(is.na(fbar(  x)[fbar(  x)>0])), NA,
      max(fbar(  x), na.rm=TRUE)))
    lim.y <- c(0, ifelse(all(is.na(yield( x)[yield( x)>0])), NA,
      max(yield( x), na.rm=TRUE)))
    lim.p <- c(0, ifelse(all(is.na(profit(x)[profit(x)>0])), NA,
      max(profit(x), na.rm=TRUE)))
    lim.s <- c(0, ifelse(all(is.na(ssb(   x)[ssb(   x)>0])), NA,
      max(ssb(   x), na.rm=TRUE)))
    lim.r=c(0, ifelse(all(is.na(rec(   x)[ssb(   x)>0])), NA,
      max(rec(   x), na.rm=TRUE)))
    ...
    if (obs)
    {
     ...
     if (!missing(ts))
     {
      ...
      if (("fbar"   %in% names(ts))) if (!all(is.na(ts[,"fbar"]  )))
        lim.h=c(0, max(c(ts[,"fbar"],   lim.h), na.rm=TRUE))
      if (("profit" %in% names(ts))) if (!all(is.na(ts[,"profit"])))
        lim.y=c(0, max(c(ts[,"profit"], lim.y), na.rm=TRUE))
      if (("yield"  %in% names(ts))) if (!all(is.na(ts[,"yield"] )))
        lim.y=c(0, max(c(ts[,"yield"],  lim.y), na.rm=TRUE))
      if (("ssb"    %in% names(ts))) if (!all(is.na(ts[,"ssb"]   )))
        lim.s=c(0, max(c(ts[,"ssb"],    lim.s), na.rm=TRUE))
      if (("rec"    %in% names(ts))) if (!all(is.na(ts[,"rec"]   )))
        lim.r=c(0, max(c(ts[,"rec"],    lim.r), na.rm=TRUE))
     }
