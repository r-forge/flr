...
## this is only temporary until complied as a package ##########################
owarn <- options()$warn.FPU
options(warn.FPU=FALSE)

##dyn.load("D:/FLR/Packages/FLBREM/inst/libs/BREM.dll")
##dyn.unload("D:/FLR/Packages/FLBREM/inst/libs/BREM.dll")
...
rec.wt  <-FLQuant(0.459,dimnames=list(age="all",year=2:30,unit="unique",area="all",season="unique"))

## run FLBREM
#t.<-FLBREM(rec,adult,control=FLBREM.control(r.index.q=1,r.index.cv=0.5,adult.q=1,adult.cv=0.2))
t.<-FLBREM(rec,adult,control=FLBREM.control(r.index.q=1,r.index.cv=0.5,adult.q=1,adult.cv=0.2,stock0=c(min=2221.9,max=2222.1,phase=-1)))
