!     Last change:  CEP  27 Sep 2010   11:35 am
...
!                   2) outputs selectivity for each year
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      MODULE CNTRL
        ...
        INTEGER :: PATCH, SEED, N_LOOPS, N_boots, SR_BOX, PR_TYPE, N_BOX, N_SCENARIOS,YR_BOX,N_GEARS(2),MODEL_TYPE, &
                   ...
                   DO_STEEPNESS(2)
        REAL (KIND=8) :: CI,AGE_PLUS(2),var_R(2),SEX_FRACTION(2)
        CHARACTER (LEN=5) :: CLASS(8)
        ALLOCATABLE BAD
        DATA CLASS/'SSBIO','YIELD','BIO_f','BIO_t','SSNUM','RECRT','Fapex','Fcurr'/
      END MODULE CNTRL

      MODULE STATISTICS
        TYPE BENCH ; REAL (KIND=8) :: FMSY, MSY, YPRMSY, SPRMSY, SPRATIOMSY, SSBMSY, FMAX, YPRMAX, SPRMAX, SPRATIOMAX, SSBMAX,F01, &
                                      YPR01,SPR01, SPRATIO01, SSBF01, &
                                      F20, YPR20, SPR20, SSB20, F30, YPR30, SPR30, SSB30, F40, YPR40, SPR40, SSB40, &
        ...
        REAL(KIND=8), SAVE ::  R(:,:),N(:,:,:),Fp(:,:,:,:),Cp(:,:,:,:),YLD(:,:,:),SSB(:,:),SSB_MALE(:,:),SSN(:,:),SSN_MALE(:,:), &
                               ...
                               U(:,:,:),SR(200,2),PLUSAGE(2),SEL_MOD(:,:,:),W(:,:,:,:),M(:,:),MATURITY(2,0:100),F100(:,:), &
                               F_CURRENT(:,:,:),YRTEMP(2),SRTEMP(2),SSBMSYTEMP(2),MSYTEMP(2),SUM_YLD(:,:),QUOTA(:,:,:),FMAX(:,:,:),&
                               LANDED_FRACTION(:,:,:,:),DISCARD_MOD(:,:,:),P_MALE(:,:,:),RAND(:,:,:),SR_mse_minimum(2)
        ...
        ALLOCATABLE R,N,F,C,YLD,SSB,SSB_MALE,SSN,SSN_MALE,BIOMASS,FBIOMASS,SELECTIVITY,F_APICAL,U,RAND,BENCHMARKS,SEL_MOD,W,M,F100,&
                    F_CURRENT,Fp,Cp,TAC,T,SUM_YLD,QUOTA,FMAX,LANDED_FRACTION,DISCARD_MOD,P_MALE
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      WRITE(*,'(7(/))')
      WRITE(*,*) '+-------------------------------------------------------------------------+'
      WRITE(*,*) '| Program PRO-2BOX.F90, Version 3.00 (September 1, 2010)                      |'
      WRITE(*,*) '|                                                                         |'
      ...
      OPEN(99,FILE='sel.OUT',STATUS='UNKNOWN')
...
!-----------------------------------------------------------------------------
      ...
      REAL (KIND=8) :: REALS(100)
      CHARACTER (LEN=30) :: CHARACTERS(100)
      SR_PENALTY=0
      WRITE(*,*) 'ENTER THE NAME OF CONTROL FILE: ' ; READ(*,*) CHARACTERS(1) ; WRITE(*,'(3/)')
1     OPEN(11,FILE=CHARACTERS(1),STATUS='OLD',ERR=10)
      ...
      CALL READIT(11,1,0,1,INTEGERS,REALS,CHARACTERS) ; S_FILE%FILETYPE=INTEGERS(1);S_FILE%FILENAME=CHARACTERS(1)
      CALL READIT(11,1,0,1,INTEGERS,REALS,CHARACTERS) ; R_FILE%FILETYPE=INTEGERS(1);R_FILE%FILENAME=CHARACTERS(1)
      CALL READIT(11,1,0,1,INTEGERS,REALS,CHARACTERS) ; T_FILE%FILETYPE=INTEGERS(1);T_FILE%FILENAME=CHARACTERS(1)
      CALL READIT(11,1,0,1,INTEGERS,REALS,CHARACTERS) ; D_FILE%FILETYPE=INTEGERS(1);D_FILE%FILENAME=CHARACTERS(1)
...
!-----------------------------------------------------------------------------
      ...
      LOGICAL :: CHECK
      INTEGER :: NPARM,B,G,Y,Y1,SCENARIO,LOOP,I,IC,N_BADLOOPS=0,FCHOICE,LUP,MAX_ITER=100
      CHARACTER (LEN=1) :: CH
      EXTERNAL OBJECTIVE_F

      CALL OPENFILES

      ALLOCATE (U(1:N_LOOPS,2,1:YEAR%PRJCT+1),RAND(1:N_LOOPS,2,1:YEAR%PRJCT+1),BENCHMARKS(1:N_LOOPS,2),&
                QUOTA(N_BOX,1:100,1:YEAR%PRJCT),FMAX(N_BOX,1:100,1:YEAR%PRJCT))
...
3       LOOP=0
        DO LUP=1,N_LOOPS
          IF(LUP==1) THEN
              WRITE(*,*) 'SCENARIO ',SCENARIO,',  DETERMINISTIC RUN'
            ELSEIF(MOD(LUP-1,10)==0) THEN
...
!         Write projections to files
          IF(LOOP==1 .AND. SCENARIO==1 .AND. Y==YEAR%LAST+1) WRITE(*,*) 'WRITING RESULTS TO FILES'
          DO B=1,N_BOX
            IC=B*20
            ...
            IF(SCENARIO==1) THEN
              ...
              IF(LOOP==1) WRITE(31+IC,1002) 'RUN #','F MSY','MSY ','Y/R MSY','S/R MSY','SPR MSY','SSB MSY','F MAX','Y/R MAX', &
                                    'S/R MAX','SPR MAX','SSB MAX','F 0.1','Y/R 0.1','S/R 0.1','SPR 0.1','SSB 0.1','F 20%',    &
                                    'Y/R 20%','S/R 20%','SSB 20%','F 30%','Y/R 30%','S/R 30%','SSB 30%','F 40%','Y/R 40%',    &
                                    'S/R 40%','SSB 40%','F 90% Y/Rmax','Y 90% Y/Rmax','90% Y/R max','S/R 90%','SSB 90%',      &
                                    'F 75% Fmax','Y 75% Fmax','Y/R 75% Fmax','S/R 75% Fmax','SSB 75% Fmax'
...
!-----------------------------------------------------------------------------
      ...
      DO B=1,N_BOX
        IC=B*20
        DO I=1,8
          FILENAME=CLASS(I)//IDBOX(B)
          open(I+IC,file=FILENAME,status='unknown')
        END DO
        FILENAME='BENCH'//IDBOX(B)
...
!-----------------------------------------------------------------------------
      ...
      REAL (KIND=8) :: U(NLOOPS,2,NYEARS),RAND(NLOOPS,2,NYEARS),RNORM,RAN1
      EXTERNAL RNORM,RAN1

      DO I=1,NLOOPS ; DO J=1,NYEARS ; DO B=1,2
            U(I,B,J)=RNORM(SEED)
            RAND(I,B,J)=RAN1(SEED)
...
!-----------------------------------------------------------------------------
      ...
      ALLOCATE (R(2,Y),N(2,AGE(1)%FIRST:A,Y),Cp(2,G,AGE(1)%FIRST:A,Y))
      ALLOCATE (Fp(2,G,AGE(1)%FIRST:A,Y),W(2,0:G,AGE(1)%FIRST:A,Y),YLD(2,G,Y),SUM_YLD(2,Y))
      ALLOCATE (F_APICAL(2,G,Y),SSB(2,Y),SSB_MALE(2,Y),SSN_MALE(2,Y),SSN(2,Y),M(2,AGE(1)%FIRST:A),P_MALE(2,AGE(1)%FIRST:A,Y))
      ALLOCATE (BIOMASS(2,Y),SELECTIVITY(2,0:G,AGE(1)%FIRST:A),SEL_MOD(2,0:G,AGE(1)%FIRST:A),DISCARD_MOD(2,0:G,AGE(1)%FIRST:A))
      ...
      N=0.0 ; F=0.0 ; FP=0.0 ; T=0.0 ; LANDED_FRACTION=0 ; SEL_MOD=0
      RETURN ; END SUBROUTINE
...
!-----------------------------------------------------------------------------
      ...
      N=0; F=0; SSB=0; W=0; M=0; SEL_MOD_MAX=0; DO_STEEPNESS=0;
!
...
!
      ...
      IF(LOOP==1) WRITE(*,*) 'READING HISTORICAL ABNDANCE ESTIMATES'
      ...
      IF(LOOP==1) WRITE(*,*) 'READING HISTORICAL FISHING MORTALITY RATE ESTIMATES'
      ...
      IF(LOOP==1) WRITE(*,*) 'READING HISTORICAL WEIGHTS'
      ...
      IF(LOOP==1) WRITE(*,*) 'READING HISTORICAL CATCHES'
      C=0
      SELECT CASE (C_FILE%FILETYPE)
        ...
        CASE (-1) ! somebody else's binary format
        CASE DEFAULT ! VPA-2BOX format
          IF(N_GEARS(1)==1.AND.N_GEARS(2)==1) THEN
      ...
      IF(LOOP==1) WRITE(*,*) 'READING HISTORICAL DISCARD FRACTIONS'
      SELECT CASE (D_FILE%FILETYPE)
          CASE (0) ! ASC-II
            ...
            END DO ; END DO
          CASE DEFAULT ! VPA-2box format
            WRITE(*,*) 'ERROR: Discard fractions at age may only be provided through an'
            write(*,*) '       ASC-ii file' ; STOP
      END SELECT ; CLOSE(10)

      IF(LOOP==1) WRITE(*,*) 'READING NATURAL MORTALITY RATES'
      SELECT CASE (M_FILE%FILETYPE)
        CASE (0) ! ASC-II
          OPEN(10,FILE=M_FILE%FILENAME,STATUS='OLD')
          DO S=1,N_BOX
            READ(10,*) i,(M(s,a),a=age(s)%first,age(s)%last)
        ...
        CASE DEFAULT ! VPA-2box format
          ...
          READ(10,REC=loop) (((FT(S,a,y),y=1,year%last),a=age(s)%first,age(s)%last),S=1,N_BOX)
          DO S=1,N_BOX ; DO A=age(s)%first,age(s)%last ; M(s,a)=DBLE(FT(s,a,year%last)) ; END DO ; END DO
          CLOSE(10)
      END SELECT

      IF(N_box>1 .AND. SR_BOX<1) THEN
        IF(LOOP==1) WRITE(*,*) 'READING TRANSFER COEFFICIENTS'
      ...
      IF(LOOP==1) WRITE(*,*) 'READING SELECTIVITY MODIFIERS'
      SELECT CASE (S_FILE%FILETYPE)
          CASE (0) ! ASC-II
            OPEN(10,FILE=S_FILE%FILENAME,STATUS='OLD')
            DO S=1,N_BOX ; DO G=1,N_GEARS(S)
              READ(10,*) i,j,(Sel_mod(s,g,a),a=age(s)%first,age(s)%last)
      ...
      IF(LOOP==1) WRITE(*,*) 'READING SPAWNER-RECRUIT PARAMETERS'
      SELECT CASE (R_FILE%FILETYPE)
          CASE (0) ! ASC-II
            ...
            DO S=1,N_BOX
              ...
              SR(4,S)=SR(4,S)*SQRT(1-SR(5,S)**2) ! determine sigma of random component from sigma of total
              IF(SR(4,S)>0) SR_mse_minimum(s)=SR(4,S)
...
!
      ...
      IF(OPENSRFILE==0) THEN ! first scenario
          ...
          IF(LOOP==1) WRITE(60,'(1x,a5,2x,2(1X,a10,4(1x,a12)))') 'RUN #',' SR_TYPE  ','PARM 1','PARM 2','STD. DEV.','    RHO   '
          IF(SR(1,1)>0) THEN ! Estimate parameters (only works for single stock and sex-specific analyses)
              CALL SR_FIT
              ...
              IF(DO_STEEPNESS(1)>0) THEN ! Convert estimated alpha and beta into steepness and R0 for reporting
                ...
                S=1; ! only works for single stock and sex specific analyses
                IF(SR_TYPE(1)==1) THEN ! Beverton and Holt
                    STEEPNESS(s)=SR(2,s)*SRTEMP(S)/(4.*SR(3,S)+SR(2,S)*SRTEMP(S)); R_virgin(S)=(SR(2,S)*SRTEMP(S)-SR(3,S))/SRTEMP(S)
                  ELSE ! Ricker
                    STEEPNESS(S)=0.2*(SR(2,S)*SRTEMP(S))**0.8; R_virgin(S)=LOG(SR(2,S)*SRTEMP(S))/(SR(3,S)*SRTEMP(S))
                ENDIF
              ENDIF
            ELSE ! do not estimate parameters; use input values
              IF(DO_STEEPNESS(1)>0 .AND. LOOP==1) THEN ! Convert input R_virgin and steepness into alpha and beta parameters (only need to do once)
          ...
          ENDIF
!          WRITE(*,*) SRTEMP(1)
!          WRITE(*,*) LOOP-1,(SR_TYPE(s),SR(2,S),SR(3,S),SR(4,S),SR(5,S),S=1,N_BOX)
          ...
          IF(ftemp(1)<0.01 .OR. ftemp(3)<0.01) THEN
              WRITE(60,'(1x,i4,3x,2(1X,I10,2(1X,D12.5),4(1x,f14.4)))') &
                                                           LOOP-1,(SR_TYPE(s),FTEMP(S),FTEMP(S+2),SR(4,S),SR(5,S),S=1,N_BOX)
            ELSE
              WRITE(60,'(1x,i4,3x,2(1X,I10,4(1x,f14.4)))') LOOP-1,(SR_TYPE(s),FTEMP(S),FTEMP(S+2),SR(4,S),SR(5,S),S=1,N_BOX)
          ENDIF
!          WRITE(*,*) 'a'
!          WRITE(*,*) LOOP-1,(SR_TYPE(s),FTEMP(S),FTEMP(S+2),SR(4,S),SR(5,S),S=1,N_BOX)
        ELSE
          IF(SR(1,1)>0) THEN ! Don't re-estimate parameters for every scenario
            IF(LOOP==1) READ(60,*)
            READ(60,*) I,(SR_TYPE(s),SR(2,S),SR(3,S),SR(4,S),SR(5,S),S=1,N_BOX)
...
!
      IF(PATCH>0) THEN
        IF(LOOP==1) WRITE(*,*) 'OVER-RIDING LAST RECRUITMENTS AND RECOMPUTING SUBSEQUENT Ns AND Fs'

        ! get recruitment replacements
        DO Y=YEAR%LAST-patch+1,YEAR%LAST
          ! Deterministic part
          IF(WEIGHT_TYPE%FIRST==0) CALL GETWAA(Y-AGE(1)%FIRST)      ! get weights from growth curve
          CALL GETSSB(y-AGE(1)%FIRST)
          XMU=0
          DO S=1,N_BOX
            rho(s)=sr(5,s) ; stddev(s)=ABS(sr(4,s))
            SELECT CASE (R_FILE%FILETYPE)
              CASE (0)     ; siggy(s)=sr(6,s)                    ! Starting residual from single entry in ASC-ii file
              CASE DEFAULT ; siggy(s)=sr(5+YEAR%LAST-PATCH,s)    ! Starting residual from binary file entry for year before patch
            END SELECT
            CALL RECRUITS(REC,S,Y)                               ! get deterministic number of recruits (REC)
            siggy(s)=rho(s)*siggy(s) ; SR(5+Y,s)=SIGGY(s)        ! carry autocorrelation through to present year
...
99        DO S=1,N_BOX ; N(S,age(s)%first,Y)=N(S,age(s)%first,Y)+ADD(S) ; END DO
          DO S=1,N_BOX
            ...
            F(S,age(s)%FIRST,Y)=GETF(U1,C(S,age(s)%FIRST,Y),TEMP,M(S,age(s)%FIRST)) ! F by Newton iteration from N and C
          ...
          DO I=Y,YEAR%LAST-1
            ...
            DO S=1,N_BOX ! abundance of cohort I years after recruitment
              ...
              IF(MODEL_TYPE==1) THEN ! diffusion and no mix
                  N(S,A+1,I+1)=N(S,A,I)*(1.0-T(S,A))*EXP(-M(S,A)-F(S,A,I))+N(O,A,I)*T(O,A)*EXP(-M(S,A)-F(S,A,I))
                ELSE ! overlap
                  N(S,A+1,I+1)=N(S,A,I)*(1.0-T(S,A))*EXP(-M(S,A)-F(S,A,I))+N(S,A,I)*T(S,A)*EXP(-M(O,A)-F(O,A,I))
              ENDIF
            END DO ! S
            DO S=1,N_BOX ! F's on cohort I years after recruitment
              IF(S==1) THEN ; O=2 ; ELSE ; O=1 ; ENDIF
              TEMP=N(S,A+1,I+1)*(1.0-T(S,A+1))+N(O,A+1,I+1)*T(O,A+1)
              F(S,A+1,I+1)=GETF(U1,C(S,A+1,I+1),TEMP,M(S,A+1))
              IF(U1>0.001) THEN ! replaced recruitment too small
...
!
      IF(LOOP==1) WRITE(*,*) 'COMPUTING PROJECTION SELECTIVITIES'
      ...
      DO S=1,N_BOX
        DO G=1,N_GEARS(S)
          DO A=age(s)%FIRST,age(s)%LAST
            IF(SEL_TYPE%FIRST>0) THEN ! Get geometric mean of historical fishing mortalities
                DO Y=SEL_TYPE%FIRST,SEL_TYPE%LAST
                  IF(Fp(S,G,A,Y)*SEL_MOD(s,g,a)>0) THEN;
                      SELECTIVITY(S,G,A)=SELECTIVITY(S,G,A)+LOG(Fp(S,G,A,Y)*SEL_MOD(s,g,a))
                    ELSE
                      SELECTIVITY(S,G,A)=SELECTIVITY(S,G,A)-10;
                  ENDIF
                END DO ! Y
                SELECTIVITY(S,G,A)=EXP(SELECTIVITY(S,G,A)/DBLE(SEL_TYPE%LAST-SEL_TYPE%FIRST+1)) ! fleet specific F
          ...
          DO A=age(s)%FIRST,age(s)%LAST
            SEL_MOD(S,0,A)=SEL_MOD(S,0,A)+SEL_MOD(S,g,A)*F_CURRENT(LOOP,S,G) ! sum F weighted input selectivities
          END DO ! A
        END DO ! G
        DO A=age(s)%FIRST,age(s)%LAST
          ZMAX(S,0)=MAX(ZMAX(S,0),SELECTIVITY(S,0,A))
          SEL_MOD_MAX(S)=MAX(SEL_MOD_MAX(S),SEL_MOD(S,0,A))
        END DO
      END DO ! S
      DO S=1,N_BOX
        DO G=0,N_GEARS(S)
          ...
          IF(SEL_TYPE%FIRST>0) THEN ! normalize geometric mean F's by maximum value to get relative selectivity
              ...
              IF(SCENARIO==1) WRITE(99,*) (SELECTIVITY(S,G,A),A=age(s)%FIRST,age(s)%LAST)
            ELSE ! User-supplied relative selectivity SELMOD
              IF(G==0) THEN
                ...
                DO A=age(s)%FIRST,age(s)%LAST ; SEL_MOD(S,0,A)=SEL_MOD(S,0,A)/SEL_MOD_MAX(S) ; END DO
              ENDIF
              DO A=age(s)%FIRST,age(s)%LAST ; SELECTIVITY(S,G,A)=SEL_MOD(S,g,A) ; END DO
              IF(SCENARIO==1) WRITE(99,*) (SELECTIVITY(S,G,A),A=age(s)%FIRST,age(s)%LAST)
          ENDIF
        END DO ! G
      END DO ! S
...
!
      IF(LOOP==1) WRITE(*,*) 'COMPUTING DISCARD MODIFICATIONS'
      DO S=1,N_BOX
        DO G=0,N_GEARS(S)
          FTEMP=0
          DO A=age(s)%FIRST,age(s)%LAST
            IF(SEL_TYPE%FIRST>0) THEN ! {geometric mean of historical landed fractions}
                DO Y=SEL_TYPE%FIRST,SEL_TYPE%LAST
                  IF(LANDED_FRACTION(s,g,a,y)>0.0) FTEMP(a)=FTEMP(a)+LOG(LANDED_FRACTION(s,g,a,y))
                END DO
...
!
      IF(LOOP==1) WRITE(*,*) 'COMPUTING HISTORICAL SSB, FISHABLE BIOMASS, ETC...'
      DO Y=YEAR%FIRST,YEAR%LAST
        IF(WEIGHT_TYPE%FIRST==0) CALL GETWAA(Y)
        CALL GETSSB(y)
        CALL GETYIELD(y)
        CALL GETBIOMASS(y)
...
! compute age of plus groups
      DO S=1,N_BOX
        IF(AGE_PLUS(S)>=AGE(S)%LAST) THEN ! Set equal to values in control file
            PLUSAGE(S)=MAX(age(s)%last,AGE_PLUS(S))
          ELSE ! Compute from average weight of plus-group given for fishery 1 in weight file
            ...
            ENDIF
            PLUSAGE(S)=MAX(age(s)%last,PLUSAGE(S)-GROWTH(S,1)%DATE/12.0) ! avg age of plus group at beginning of year
        ENDIF
      END DO
...
!
      ...
      IF(LOOP==1) WRITE(*,*) 'END OF SUBROUTINE GETDATA'
...
!                  this is the equilibrium approximation for historical plus-age, which is crude if F or R changes thru time
                   Ta=DBLE(age(s)%LAST)-1.0+1.0/(1.0-EXP(-m(s,a)-F(s,a,y)))+GROWTH(S,G)%DATE/12.0
               ENDIF
             ELSE
      ...
      DO B=1,N_BOX
        ...
        DO A=AGE(b)%FIRST,AGE(b)%LAST
          IF(MODEL_TYPE==1) THEN ! diffusion
              N(B,A+1,Y+1)=(N(b,A,Y)*(1.0-T(b,a))+N(o,A,Y)*T(o,a))*EXP(-M(b,A)-F(b,a,y))
            ELSE ! overlap
              N(B,A+1,Y+1)=N(b,A,Y)*((1.0-T(b,a))*EXP(-M(b,A)-F(b,a,y))+T(b,a)*EXP(-M(o,A)-F(o,a,y)))
          ENDIF
        END DO
      END DO
...
!     update age of plus group
      DO B=1,N_BOX
        ...
        IF(MODEL_TYPE==1) THEN ! diffusion
            A=AGE(b)%LAST-1
            TEMP(b)=DBLE(age(b)%LAST)*(N(b,A,Y)*(1.0-T(b,a))+N(o,A,Y)*T(o,a))*EXP(-M(b,A)-F(b,a,y))+ &
                    ((PLUSAGE(b)+1.0)*N(b,A+1,Y)*(1.0-T(b,a+1))+(PLUSAGE(o)+1.0)*N(o,A+1,Y)*T(o,a+1))*EXP(-M(b,A+1)-F(b,a+1,y))
          ELSE ! overlap
            TEMP(b)=DBLE(age(b)%LAST)*N(b,age(b)%last,Y+1)+(PLUSAGE(b)+1.0)*N(b,age(b)%LAST+1,Y+1)
        ENDIF
      END DO
      DO B=1,N_BOX
        N(b,age(b)%last,Y+1)=N(b,age(b)%last,Y+1)+N(b,AGE(b)%last+1,Y+1) ; PLUSAGE(b)=MAX(AGE(b)%LAST,TEMP(b)/N(b,AGE(b)%last,Y+1))
      END DO
...
!     begin a new cohort (recruits)
      ...
      IF(AGE(1)%FIRST==0) THEN ! Get SSB at beginning of next forecasted year (age 0 produced at beginning of year)
        ...
        CALL GETWAA(Y+1); CALL GETSSB(y+1)
      ENDIF
      XMU=0
      DO B=1,N_BOX
        IF(Y==YEAR%LAST) THEN
          SELECT CASE (R_FILE%FILETYPE)
            CASE (0)     ; ETA(b)=sr(6,b) ! from single value in ASC-ii file
            CASE DEFAULT ; ETA(b)=sr(5+Y,b) ! from binary file or patch replacement for last year in data
          END SELECT
        ENDIF
        ...
        IF(REC>0 .AND. LOOP>1) THEN
          IF(SR_TYPE(b)==6) THEN
            ...
            ELSE
              ...
              IF(ERRORTYPE==1) THEN
                  xmu=log(rec)                                ! mu = E[ln(R)] = ln(E[R])-mse/2 where E is expectation
                  rec=exp(xmu+ETA(iBOX))                 ! stochastic recruitment value with mean = deterministic value
                ELSE
                  REC=REC+ETA(iBOX)
              ENDIF
          ENDIF
          N(B,age(b)%first,Y+1)=REC
        ENDIF
      END DO
      IF(SR_BOX>0.AND.SR_TYPE(BOX%STORE)/=6) THEN ; DO B=1,N_BOX ; N(B,age(b)%first,Y+1)=REC*SEX_FRACTION(B) ; END DO ; ENDIF !sex
...
!-----------------------------------------------------------------------------
      USE CNTRL ; USE STATISTICS ; IMPLICIT NONE
      INTEGER :: B,Y,COUNTS,NPARM,MAX_ITER=100,N_CYCLES
      REAL (KIND=8) :: maxssb,minssb,aveSSB,maxR,sumR,logsumr,PARM(100),p(100,99),x(100),O(100),PB(100),FUNC,REC,SSBTEMP,REGRESS,ss
      EXTERNAL OBJECTIVE_SR
      DO Y=1,YEAR%LAST
        CALL GETWAA(Y) ; CALL GETSSB(Y)
      END DO
      IF(SR_BOX>0) THEN ; N_CYCLES=1 ; ELSE ; N_CYCLES=N_BOX ; ENDIF
      DO B=1,N_CYCLES
        maxR=-1 ; maxSSB=-1 ; minSSB=1.0D+32 ; aveSSB=0.0 ; sumR=0.0 ; logsumr=0.0 ; var_R(b)=0.0 ; COUNTS=0 ; SR(5,B)=0.0
        regress=0
        ...
        IF(SR_TYPE(b)==1) THEN ! {Beverton and Holt curve}
          ...
          ELSEIF(SR_TYPE(b)==4.OR.SR_TYPE(b)==5) THEN ! {Two-line curve based on geometric mean recruitment}
            ...
            ENDIF
          ELSE ! {constant average recruitment}
            SR(2,b)=sumR/dble(counts)
            SR(4,b)=SQRT(var_R(b)*DBLE(COUNTS)/(DBLE(COUNTS)-1))
        ENDIF
        IF(SR(4,b)>0 .AND. SR_TYPE(b)>2 .AND. SR_TYPE(b)<6) THEN ! Estimate standard error for two-line models
            SR(4,b)=0.0
            DO Y=REC_YEAR(1),REC_YEAR(2)
              IF (Y>=YEAR%FIRST+age(b)%FIRST) THEN
                IF(SR_BOX>0) THEN ; REC=N(1,age(1)%FIRST,Y)+N(2,age(2)%FIRST,Y) ; ELSE ; REC=N(B,age(b)%FIRST,Y) ; ENDIF
                SELECT CASE (SR_BOX)
                  ...
                  CASE DEFAULT ; SSBTEMP=SSB(B,Y-age(b)%FIRST)
                END SELECT
                IF(SSBTEMP<MAXSSB) THEN ; O(1)=SR(2,B)*SSBTEMP/MAXSSB ; ELSE ; O(1)=SR(2,B) ; ENDIF
                IF(ERRORTYPE==1) THEN
...
!     Fmax: maximum yield per recruit
      ...
      END DO
      IF(SR_BOX>0) BENCHMARKS(LOOP,2)%FMAX=BENCHMARKS(LOOP,1)%FMAX

!     F0.1: (F where slope = 0.1 slope at F=0) -- for two-areas this is obtained assuming the other area is fished at
!          Fmax. Eventually this will be replaced with a root-bracketing algorithm that considers both F's simultaneously)
      ! Forward difference to find slope at origin = (y/r{x}-0)/x
      IF(LOOP==1) WRITE(*,*) '     F0.1'
      DO B=1,NPARM ; X(b)=0.000001 ; END DO  ; CALL OBJECTIVE_YR(X,func,NPARM)
      IF(SR_BOX>0) THEN ! Sex-specific
          SLOPE(1)=0.1*ABS(FUNC)/0.000001 !numerical approx. to slope of y/r curve at origin
        ELSE ! Not sex-specific
          DO B=1,NPARM ; SLOPE(B)= 0.1*YRTEMP(B)/0.000001 ; END DO
      END IF
 ...
 !       DO S=1,NPARM ; X(S)=1.0 ; END DO ; CALL OBJECTIVE_YR(X,FUNC,NPARM)
        OLDF=ABS(FUNC)
        DO WHILE (FINISH(1)==0)
          IF(X(B)<0.001) THEN
              EXIT
            ELSE
              X(B)=X(B)-0.001 ; CALL OBJECTIVE_YR(X,FUNC,NPARM)
          ENDIF
          IF((OLDF-ABS(FUNC))/0.001>SLOPE(B)) EXIT
          OLDF=ABS(FUNC)
        END DO
        BENCHMARKS(LOOP,B)%F01=X(B) ; BENCHMARKS(LOOP,B)%YPR01=YRTEMP(B) ; BENCHMARKS(LOOP,B)%SPR01=SRTEMP(B)
        IF(SR_BOX>0) THEN ! Sex-specific, so only one pass made
          BENCHMARKS(LOOP,2)%F01=BENCHMARKS(LOOP,1)%F01
          BENCHMARKS(LOOP,2)%YPR01=YRTEMP(2) ; BENCHMARKS(LOOP,2)%SPR01=SRTEMP(2)
        ENDIF
      END DO

!     F at 90% max Y/R (w/mixing, many combinations of F(1) and F(2) will give 90% maxY/R, decrementing each F equally is one way)
      IF(NPARM>1) THEN;
          BENCHMARKS(LOOP,1)%F90=0; BENCHMARKS(LOOP,2)%F90=0;
          BENCHMARKS(LOOP,1)%YPRf90=0; BENCHMARKS(LOOP,2)%YPRf90=0;
          BENCHMARKS(LOOP,1)%SPRf90=0; BENCHMARKS(LOOP,2)%SPRf90=0;
        ELSE
          ...
          END SELECT
          DO WHILE (FINISH(1)==0)
            X(1)=X(1)-0.001 ; CALL OBJECTIVE_YR(X,FUNC,NPARM)
            IF(X(1)<0.001 .OR. ABS(FUNC)<=0.9*TEMP) EXIT
          END DO
          DO B=1,N_BOX
            BENCHMARKS(LOOP,B)%F90=X(B) ; BENCHMARKS(LOOP,B)%YPRf90=YRTEMP(B) ; BENCHMARKS(LOOP,B)%SPRf90=SRTEMP(B)
          END DO
          IF(SR_BOX>0) BENCHMARKS(LOOP,2)%F90=BENCHMARKS(LOOP,1)%F90
      ENDIF
...
!     F at 75% Fmax
      ...
      END DO
      IF(SR_BOX>0) BENCHMARKS(LOOP,2)%F75Fmax= BENCHMARKS(LOOP,1)%F75Fmax

!     F where spawning potential ratio is 20%, 30% or 40% -- for two areas this is obtained assuming other area fished at Fmax
      ...
      DO B=1,NPARM
         DO S=1,NPARM ; X(S)=BENCHMARKS(LOOP,S)%FMAX+0.2 ; END DO
         DO WHILE (FINISH(2)==0)
           ...
           SELECT CASE (SR_BOX)
               ...
               CASE DEFAULT ; RATIO=SRTEMP(B)/VIRGIN(B)                           ! not sex-specific
           END SELECT
           IF(RATIO>=0.201 .AND. FINISH(4)==0) THEN
               BENCHMARKS(LOOP,b)%F20=X(b) ; BENCHMARKS(LOOP,b)%SPR20=SRTEMP(b) ; BENCHMARKS(LOOP,b)%YPR20=yrtemp(b) ; FINISH(4)=1
               IF(SR_BOX>0) THEN ; BENCHMARKS(LOOP,2)%SPR20=SRTEMP(2) ; BENCHMARKS(LOOP,2)%YPR20=yrtemp(2) ; ENDIF
             ELSEIF(RATIO>=0.301 .AND. FINISH(3)==0) THEN
               BENCHMARKS(LOOP,b)%F30=X(b) ; BENCHMARKS(LOOP,b)%SPR30=SRTEMP(b) ; BENCHMARKS(LOOP,b)%YPR30=yrtemp(b) ; FINISH(3)=1
               IF(SR_BOX>0) THEN ; BENCHMARKS(LOOP,2)%SPR30=SRTEMP(2) ; BENCHMARKS(LOOP,2)%YPR30=yrtemp(2) ; ENDIF
             ELSEIF(RATIO>=0.401 .AND. FINISH(2)==0) THEN
               BENCHMARKS(LOOP,b)%F40=X(b) ; BENCHMARKS(LOOP,b)%SPR40=SRTEMP(b) ; BENCHMARKS(LOOP,b)%YPR40=yrtemp(b) ; FINISH(2)=1
               IF(SR_BOX>0) THEN ; BENCHMARKS(LOOP,2)%SPR40=SRTEMP(2) ; BENCHMARKS(LOOP,2)%YPR40=yrtemp(2) ; ENDIF
           ENDIF
         END DO
         FINISH=0
      END DO
      IF(SR_BOX>0) THEN ! Sex-specific has only one apical F
...
!     msy calculations directly from spawner-recruit and yield-per-recruit statistics
      ...
      IF(SR_TYPE(1)==1 .OR. SR_TYPE(1)==2) THEN
          ...
          DO B=1,N_BOX
            ...
            BENCHMARKS(LOOP,B)%MSY=MSYTEMP(B) !/1000.0
            BENCHMARKS(LOOP,B)%SPRMSY=SRTEMP(B) ; BENCHMARKS(LOOP,B)%SSBMSY=SSBMSYTEMP(B) !/1000.
          END DO
          IF(SR_BOX>0) BENCHMARKS(LOOP,2)%FMSY=PARM(1) ! Sex-specific has only one apical F
        ...
        ELSEIF(SR_TYPE(1)>2 .OR. SR_TYPE(1)<6) THEN
          ...
          ! by making long-term projections with F values progressively lower than Fmax
          DO B=1,N_BOX
            RMAX(b)=SR(2,B)
            IF(BENCHMARKS(LOOP,B)%sprmax>(1000.0*SR(3,b)/SR(2,b)) ) THEN  ! Fmsy equivalent to Fmax
              ...
              ELSE ! Fmax not sustainable
                ...
                DO WHILE (FINISH(2)==0)
                  ...
                  END SELECT
                  IF(RATIO>=1000.0*SR(3,b)/SR(2,b) .AND. FINISH(2)==0) THEN
                      BENCHMARKS(LOOP,b)%FMSY=X(b) ; BENCHMARKS(LOOP,b)%SPRMSY=SRTEMP(b) ;
                      BENCHMARKS(LOOP,B)%MSY=yrtemp(b)*rmax(b)/1000.0 ; FINISH(2)=1
                ...
                END DO
            ENDIF
            IF(SR_BOX>0)  BENCHMARKS(LOOP,2)%Fmsy=BENCHMARKS(LOOP,1)%Fmsy ! Sex-specific has only one apical F
          END DO

        ELSE ! constant recruitment
          DO B=1,N_BOX
            rmax(b)=SR(2,B)
...
!-----------------------------------------------------------------------------
      ...
      REAL (KIND=8) :: PARM(NPARM),z(2),ftemp(0:100),ntemp(2),d(2),e(2),SURV(2), &
                       WEIGHT(2,0:1),func,Ta,tl,PENALTY,temp
      func=0 ; yrtemp=0. ; srtemp=0. ; PENALTY=0
      NTEMP=SEX_FRACTION ! NOTE: SEX_FRACTION==1 if SR_BOX<1
      DO S=1,NPARM
        IF(PARM(S)<0.0) PENALTY=PENALTY+1.0D10*(1+PARM(S)**2)
        IF(PARM(S)>3.0) PENALTY=PENALTY+1.0D10*(1+(PARM(S)-3.0)**2)
      END DO
      IF(SR_BOX>0) THEN
      ...
      DO A=AGE(1)%FIRST,MAXAGE
       DO S=1,N_BOX
         ...
         ENDIF
         Ftemp(s)=MAX(PARM(I),0)*SELECTIVITY(S,0,ag) ; Z(S)=M(S,ag)+Ftemp(S) ; Ftemp(s)=Ftemp(s)*DISCARD_MOD(s,0,ag)
       ...
       DO S=1,N_BOX
         ...
         IF(s==1) THEN ; O=2 ; ELSE ; O=1 ; ENDIF
         IF(MODEL_TYPE==1) THEN ! diffusion-- ntemp(s) represents numbers in area s (regardless of where they were born)
...
!           {Compute msy for combined stocks or focusing on one stock in particular}
            IF(SR_TYPE(s)==1) THEN ! Beverton and Holt
                SSBMSYtemp(s)=sr(2,s)*TEMP/1000.0-sr(3,s)
              ELSEIF(SR_TYPE(s)==2) THEN ! Ricker
                SSBMSYtemp(s)=LOG(sr(2,s)*TEMP/1000.0)/sr(3,s)
            ENDIF
...
!-----------------------------------------------------------------------------
      ...
      REAL (KIND=8) :: PARM(NPARM),RSS,FUNC,PRED,maxR,PENALTY,NEGLOGLIKELIHOOD,MSE,TEMP,REC
      RSS=0 ; FUNC=0 ; COUNTS=0 ; MAXR=0; PENALTY=0;
      DO I=1,NPARM ; IF(PARM(I)<=0) THEN ; FUNC=1.D32 ; RETURN ; ENDIF ; END DO

      DO Y=1,YEAR%LAST

        SELECT CASE (SR_BOX)
            CASE (1) ;     REC=N(1,AGE(1)%first,Y)+N(2,AGE(1)%first,Y)
            CASE (2) ;     REC=N(1,AGE(1)%first,Y)+N(2,AGE(1)%first,Y)
            CASE (3) ;     REC=N(1,AGE(1)%first,Y)+N(2,AGE(1)%first,Y)
            CASE DEFAULT ; REC=N(BOX%STORE,AGE(1)%first,Y)            
        END SELECT
        ...
        IF (YS>=YEAR%FIRST .AND. Y>=REC_YEAR(1) .AND. Y<=REC_YEAR(2)) THEN
          ...
          RSS=RSS+(PRED-REC)**2. ; COUNTS=COUNTS+1
        ENDIF

      END DO
      MSE=MAX(RSS/DBLE(COUNTS),1.0D-32)
      IF(SR(4,BOX%STORE)>0) SR(4,BOX%STORE)=MAX(SQRT(MSE),SR_mse_minimum(BOX%STORE))
      NEGLOGLIKELIHOOD=0.5*DBLE(COUNTS)*(1.0+LOG(MSE))
...
!-----------------------------------------------------------------------------
      ...
      END DO
      DO A=age(b)%FIRST,age(b)%Last ; z(b,a)=z(b,a)+m(b,a) ; e(b,a)=EXP(-z(b,A)) ; END DO
      IF(SR_BOX>0) THEN ; DO A=age(o)%FIRST,age(o)%Last ; z(o,a)=z(o,a)+m(o,a) ; e(o,a)=EXP(-z(o,A)) ; END DO ; ENDIF
...
! This routine uses growth coefficients from fishery 1
      ...
      DO s=1,N_BOX
        ...
        DO A=age(s)%FIRST,age(s)%LAST
          Z=M(S,A)+F(s,a,y) ; E=(1-EXP(-Z))/Z/1000.0
...
!-----------------------------------------------------------------------------
      ...
      DO s=1,N_BOX
        ...
        DO A=age(s)%FIRST+I,age(s)%LAST
          E=EXP((-M(S,A)-F(s,a,y))*GROWTH(S,0)%DATE/12.0)
          IF(MODEL_TYPE==1) THEN
              SSB(S,Y)=SSB(S,Y)+(N(S,A,Y)*(1.0-T(s,a))+N(o,A,Y)*T(o,a))*MATURITY(S,A)*E*W(S,0,A,Y)/1000.0
              SSN(S,Y)=SSN(S,Y)+(N(S,A,Y)*(1.0-T(s,a))+N(o,A,Y)*T(o,a))*MATURITY(S,A)*E
            ELSE
              EO=EXP((-M(O,A)-F(O,a,y))*GROWTH(O,0)%DATE/12.0)
              SSB(S,Y)=SSB(S,Y)+N(S,A,Y)*((1.0-T(s,a))*E+T(S,a)*EO)*MATURITY(S,A)*W(S,0,A,Y)/1000.0
...
!-----------------------------------------------------------------------------
      ...
      DO S=1,N_BOX
        ...
        DO J=1,8
          ...
          FILNAME=CLASS(J)//IDBOX(S)
          CALL SUMMARIZE(J+IC,I,NCOL,ci,FILNAME,N_boots,NS,YEAR%display)
        END DO

        I=20*s ; NCOL=YEAR%PRJCT
        WRITE(*,*) 'SUMMARIZING SSB RELATIVE TO SSB AT MSY'
        REWIND(1+ic)
        FILNAME='SSBMSY'//IDBOX(S)
        CALL SUMMARIZE(1+IC,I,NCOL,ci,FILNAME,N_boots,N_SCENARIOS,YEAR%display)
        WRITE(*,*) 'SUMMARIZING SSB RELATIVE TO SSB AT F01'
        REWIND(1+ic)
        FILNAME='SSBF01'//IDBOX(S); i=81+10*s;
        CALL SUMMARIZE(1+IC,I,NCOL,ci,FILNAME,N_boots,N_SCENARIOS,YEAR%display)
        WRITE(*,*) 'SUMMARIZING SSB RELATIVE TO SSB AT F20'
        REWIND(1+ic)
        FILNAME='SSBF20'//IDBOX(S); i=82+10*s;
        CALL SUMMARIZE(1+IC,I,NCOL,ci,FILNAME,N_boots,N_SCENARIOS,YEAR%display)
        WRITE(*,*) 'SUMMARIZING SSB RELATIVE TO SSB AT F30'
        REWIND(1+ic)
        FILNAME='SSBF30'//IDBOX(S); i=83+10*s;
        CALL SUMMARIZE(1+IC,I,NCOL,ci,FILNAME,N_boots,N_SCENARIOS,YEAR%display)
        WRITE(*,*) 'SUMMARIZING SSB RELATIVE TO SSB AT F40'
        REWIND(1+ic)
        FILNAME='SSBF40'//IDBOX(S); i=84+10*s;
        CALL SUMMARIZE(1+IC,I,NCOL,ci,FILNAME,N_boots,N_SCENARIOS,YEAR%display)
...
12            continue
       ...
       IF(IFILE.EQ.20.or.IFILE.EQ.40) THEN
        WRITE(7,'(13X,A15,1X,99(A12,2X))') 'MEASURE        ','LOWER CL ','MEDIAN  ','UPPER CL ','AVERAGE ','  RUN 0  ','STD. DEV.'
        ...
        write(7,101) 'SSB AT MSY     ',low(6),median(6),hi(6),mean(6),X(6,1),std(6)
        ...
        write(7,101) 'SSB at Fmax    ',low(11),median(11),hi(11),mean(11),X(11,1),std(11)
        ...
        write(7,101) 'SSB at F0.1    ',low(16),median(16),hi(16),mean(16),X(16,1),std(16)
        write(7,101) 'F 20% SPR      ',low(17),median(17),hi(17),mean(17),X(17,1),std(17)
        write(7,101) 'Y/R at F20     ',low(18),median(18),hi(18),mean(18),X(18,1),std(18)
        write(7,101) 'S/R at F20     ',low(19),median(19),hi(19),mean(19),X(19,1),std(19)
        write(7,101) 'SSB at F20     ',low(20),median(20),hi(20),mean(20),X(20,1),std(20)
        write(7,101) 'F 30% SPR      ',low(21),median(21),hi(21),mean(21),X(21,1),std(21)
        write(7,101) 'Y/R at F30     ',low(22),median(22),hi(22),mean(22),X(22,1),std(22)
        write(7,101) 'S/R at F30     ',low(23),median(23),hi(23),mean(23),X(23,1),std(23)
        write(7,101) 'SSB at F30     ',low(24),median(24),hi(24),mean(24),X(24,1),std(24)
        write(7,101) 'F 40% SPR      ',low(25),median(25),hi(25),mean(25),X(25,1),std(25)
        write(7,101) 'Y/R at F40     ',low(26),median(26),hi(26),mean(26),X(26,1),std(26)
        write(7,101) 'S/R at F40     ',low(27),median(27),hi(27),mean(27),X(27,1),std(27)
        write(7,101) 'SSB at F40     ',low(28),median(28),hi(28),mean(28),X(28,1),std(28)
        ...
        write(7,101) 'SSB 90% max Y/R',low(33),median(33),hi(33),mean(33),X(33,1),std(33)
        ...
        write(7,101) 'SSB at 75% Fmax',low(38),median(38),hi(38),mean(38),X(38,1),std(38)
        IF(SR_BOX>0) WRITE(7,*) '     Note: All statistics are sex-specific except F and SSB values.'
       ELSE
         IF(IFILE==28 .OR. IFILE==48) THEN
            IF(SCENARIO.EQ.1) WRITE(7,'(1X,99(A10,4X))') 'SCENARIO','FLEET   ','LOWER CL ','MEDIAN  ','UPPER CL ','AVERAGE ',  &
                                                     ' RUN 0  ','STD. DEV.'
            DO j=1,ncol ; write(7,100) scenario,j,low(j),median(j),hi(j),mean(j),X(j,1),std(j) ; END DO
           ELSE
            IF(SCENARIO.EQ.1) WRITE(7,'(1X,99(A10,4X))') 'SCENARIO','YEAR    ','LOWER CL ','MEDIAN  ','UPPER CL ','AVERAGE ',  &
                                                     ' RUN 0  ','STD. DEV.'
...
!----------------------------------------------------------------------------
      ...
      REAL (KIND=8) :: PARM(NPARM+1),p(NPARM+1,NPARM),x(NPARM),O(NPARM+1),ftol,RSS,RNORM,RAN1,PDEV,PB(1)
      EXTERNAL OBJECTIVE
      MP=NPARM+1
      ndim=NPARM
      ntry=0
