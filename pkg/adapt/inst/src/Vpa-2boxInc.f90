!     Last change:  CEP  15 Jan 102    8:48 am
!-----------------------------------------------------------------
...
!     initialize all data (negative values mean no data)
      ...
      IF(MODEL_TYPE==0) THEN ; OPTION(1)=2 ! Combine data from two areas for 1 vpa
        ELSE IF(MODEL_TYPE==2 .AND. OPTION(1)==1) THEN; MODEL_TYPE=1
      END IF
      NBOX=OPTION(1)

!     read general specifications
      !FLR WRITE(*,*) 'READING GENERAL SPECIFICATIONS IN DATA FILE'
      ...
      CALL BLANK(10,RECORD)
      READ(10,*,IOSTAT=STAT,ERR=1000,END=1000) FIRSTAGE,LASTAGE,PLUSGROUP 
      IF(STAT.NE.0) GOTO 1000   
      IF(PLUSGROUP.GE.0) PLUSGROUP=MIN(LASTAGE,PLUSGROUP)
...
!     read area-specific data
      DO 1 BOX=1,NBOX
        ...
        CALL BLANK(10,RECORD)
        READ(10,*) (FECUNDITY(box,a),a=FIRSTAGE,PLUSGROUP)
!       read catch data
!FLR    WRITE(*,*) 'READING CATCH DATA' ; COUNT=0
        COUNT=0
        CALL BLANK(10,RECORD)
...
!FLR    WRITE(*,*) 'READING CPUE DATA' ; CH='FISHERY  '
30      CALL BLANK(10,RECORD)
        ...
        IF(PDF_EFFORT(g,box)/=0.AND.OPTION(4)<=0.AND.PDF_EFFORT(g,box)/=12) N_Qs=N_Qs+1  ! number of Q's estimated in conc. like.
        CALL CHECKDIMENSION(AGE_EFFORT(1,g,box),As,PLUSGROUP,'AGE ',RECORD)
        CALL CHECKDIMENSION(AGE_EFFORT(2,g,box),As,PLUSGROUP,'AGE ',RECORD)
!       hardwire
        IF(ITEMP(2)<0 .AND. NBOX>1) THEN ; BOTH_STOCKS(g,box)=1 ; ELSE ; BOTH_STOCKS(g,box)=-1 ; ENDIF
        IF(PDF_EFFORT(g,box) <= 0) THEN
          WRITE(*,*) 'YOU HAVE CHOSEN NOT TO USE THE CPUE INDEX FOR FISHERY ',G
        ENDIF
        GOTO 30
...
40      CALL BLANK(10,RECORD)
        ...
        IF(PDF_EFFORT(g,box)==1 .and. BIO_EFFORT(g,box)>2) EFFORT_DATA(g,box,y)=EFFORT_DATA(g,box,y)+0.00001
        GOTO 40
...
!       read effort selectivity data
        ...
        DO 42 A=AGE_EFFORT(1,g,box),AGE_EFFORT(2,g,box)
...
49      G=G !FLR WRITE(*,*) 'READING WEIGHT AT AGE DATA FOR FISHERIES AND SURVEYS'
50      CALL BLANK(10,RECORD)
        ...
        END DO
        GOTO 50
59      Y=Y !FLR WRITE(*,*) 'READING WEIGHT AT AGE DATA FOR SPAWNING STOCK BIOMASS'
...
1     CONTINUE
      LASTAGE=PLUSGROUP
      !FLR WRITE(*,*) 'FINISHED READING DATA FILE' ; WRITE(*,'(i5,a21)') RECORD,' RECORDS READ IN FILE'
      IF(COUNTNEGS<5) THEN ; WRITE(*,*) 'INPUT ERROR: Too few categories in data file (you are missing some -1)'; PAUSE;STOP; ENDIF
      IF(PDF_TAG>0) CALL READ_TAG ; CLOSE(10) ; RETURN

999   WRITE(*,'(1X,A21,A50)') 'ERROR: The data file ',INFILE(1)
...
!-----------------------------------------------------------------
      USE STATISTICS ; USE DATUM
      USE PARAMETERS, ONLY : INFILE,OPTION,SCALEs,MODEL_TYPE,NBOX,SIGMA_TAG_MOD,FISHING_PRESSURE
      IMPLICIT NONE
      INTEGER :: RECORD=0,BREL,BREC,A,YREL,YREC,Y_REL,J,JMIN
      REAL (KIND=8) :: SUM_TAG_FISH,SUM_FISH=0
!     initialize all data (negative values mean no data)
      RDATA = 0.0 ; NEWTAGS=0.0  ; SUMRDATA=0 ; TAG_TIME=0 ; SIGMA_TAG=-1
      DO J=1,12 ; SUM_FISH=SUM_FISH+FISHING_PRESSURE(J) ; END DO
      OPEN(10,FILE=INFILE(6),STATUS='OLD',ERR=999)
      !FLR WRITE(*,*) 'READING TAGGING DATA'
1     CALL BLANK(10,RECORD)
      READ(10,*,END=2) BREL ; IF(BREL<=0) THEN ; GOTO 2 ; ELSE ; BACKSPACE(10) ; ENDIF
      READ(10,*,END=2) BREL,YREL ; IF(YREL<=0) THEN ; GOTO 2 ; ELSE ; BACKSPACE(10) ; ENDIF
      Y_REL=YREL-DISPLAYYEAR+1
      READ(10,*) BREL,YREL,A,NEWTAGS(brel,a,y_rel),TAG_TIME(brel,a,y_rel),SIGMA_TAG(brel,a,y_rel)
      SIGMA_TAG(brel,a,y_rel)=SIGMA_TAG(brel,a,y_rel)/sqrt(SIGMA_TAG_MOD)
      JMIN=INT(TAG_TIME(brel,a,y_rel))+1 ; SUM_TAG_FISH=0
      DO J=JMIN+1,12 ; SUM_TAG_FISH=SUM_TAG_FISH+FISHING_PRESSURE(J) ; END DO
      SUM_TAG_FISH=SUM_TAG_FISH+DBLE(JMIN)-TAG_TIME(brel,a,y_rel)
      FISH_TIME(brel,a,y_rel)=SUM_TAG_FISH/SUM_FISH
      TAG_TIME(brel,a,y_rel)=1.0-TAG_TIME(brel,a,y_rel)/12.0
      GOTO 1
2     CALL BLANK(10,RECORD)
      READ(10,*,END=100) BREL ; IF(BREL<=0) THEN ; GOTO 100 ; ELSE ; BACKSPACE(10) ; ENDIF
      READ(10,*,END=100) BREL,YREL ; IF(YREL<=0) THEN ; GOTO 100 ; ELSE ; BACKSPACE(10) ; ENDIF
      Y_REL=YREL-DISPLAYYEAR+1
      READ(10,*) BREL,YREL,A,BREC,(RDATA(brel,brec,a,y_rel,yrec),YREC=FIRSTYEAR,LASTYEAR)
      DO YREC=FIRSTYEAR,LASTYEAR
        IF(YREC<Y_REL .AND. RDATA(brel,brec,a,y_rel,yrec)>0) THEN
            WRITE(*,*) 'ERROR: There are recaptures in year ',YREC+DISPLAYYEAR-1,' from tags'
            WRITE(*,*) '       released in year',YREL,', age ',A,' area ',BREL
            PAUSE; STOP
          ELSE
        ENDIF
        SUMRDATA(BREL,A,Y_REL)=SUMRDATA(BREL,A,Y_REL)+RDATA(brel,brec,a,y_rel,yrec)
      END DO
      IF(SUMRDATA(BREL,A,Y_REL)>NEWTAGS(brel,a,y_rel)) THEN
        WRITE(*,*) 'ERROR: There are more recaptures than releases for area',brel
        WRITE(*,*) '       year,',y_rel,', age',a
        PAUSE; STOP
      ENDIF
      GOTO 2
100   WRITE(*,*) 'END OF TAGGING FILE REACHED'
      CLOSE(10) ; RETURN
999   WRITE(*,'(1X,A25,A50)') 'ERROR: The tag data file ',INFILE(6)
      WRITE(*,'(8X,A55)') 'does not exist or is being used by another application.'  ; PAUSE ; STOP
      END
...
!------------------------------------------------------------------------------
      ...
      USE DATUM
      IMPLICIT NONE
      INTEGER I,K,A,Y,NPARM,G
      REAL (KIND=8) :: X(NPARM), TEMP, DUMMY
...
! {terminal-age f-ratios)
       DO 20 Y=FIRSTYEAR,LASTYEAR
         IF(FIRSTCALL==0.and.Y==FIRSTYEAR.AND.((PARM_SPECS(I,4)<0.and.PARM_SPECS(I,4)>-1.0) .or. &
           (PARM_SPECS(I,4)>0.0001 .AND. PARM_SPECS(I,4)<0.3))) GOTO 992
...
! {natural mortality}
       DO A=FIRSTAGE,LASTAGE
         IF(FIRSTCALL==0.and.a==FIRSTAGE.AND.((PARM_SPECS(I,4)<0.and.PARM_SPECS(I,4)>-1.0) .or. &
           (PARM_SPECS(I,4)>0.0001 .AND. PARM_SPECS(I,4)<0.3))) GOTO 993
...
! {transfer coefficients)
       DO A=FIRSTAGE,LASTAGE
         IF(FIRSTCALL==0.and.a==FIRSTAGE.AND.((PARM_SPECS(I,4)<0.and.PARM_SPECS(I,4)>-1.0) .or. &
           (PARM_SPECS(I,4)>0.0001 .AND. PARM_SPECS(I,4)<0.3))) GOTO 994
       ...
       DO A=1,5
         IF(FIRSTCALL==0) THEN
           IF(PDF_STOCKRECRUIT==0 .AND. (PARM_SPECS(I,4)>0.0001 .OR. PARM_SPECS(I,4)<0)) THEN
             ...
             WRITE(*,*) '       for these parameters to 0 or activate the stock-recruit penalty' ; PAUSE; STOP
            ...
            ELSEIF(PDF_STOCKRECRUIT <0 .AND. A==5 .AND. PARM_SPECS(I,4)>=0.001) THEN
             ...
             WRITE(*,*) '       concentrated method by making the control file entry positive.' ; PAUSE; STOP
            ELSEIF((PARM_SPECS(I,4)>0.0001 .AND. PARM_SPECS(I,4)<.3) .OR. PARM_SPECS(I,4)<0) THEN
             WRITE(*,*) 'ERROR: Stock-recruit parameters may not be linked to one another or to transfer rates'  ; PAUSE; STOP
           ENDIF
         ENDIF
       ...
       DO G=1,NGEARS(box)
         IF(FIRSTCALL==0) THEN
           ...
           IF(INT(CV_OVERIDE)==999 .AND. (PARM_SPECS(I,4)>0.0001 .OR. PARM_SPECS(I,4)<0)) THEN
             ...
             WRITE(*,*) '       and must be set to a constant (say 1.0) with the method indicator set to 0.' ; PAUSE; STOP
            ELSEIF(PDF_EFFORT(G,box)==0 .AND. PARM_SPECS(I,4)>0.0001) THEN
             ...
             WRITE(*,*) '       cannot be estimated and the method indicator should be set to 0 or -0.1).'   ; PAUSE; STOP
...
! {fraction of animals surviving the initial tagging procedure}
         DO A=FIRSTAGE,LASTAGE
           IF(ICALL==0 .and. a==FIRSTAGE .AND. (PARM_SPECS(I,4)<0 .or. (PARM_SPECS(I,4)>0.0001 .AND. PARM_SPECS(I,4)<0.3))) GOTO 996
           CALL GETPAR(I,K,X,NPARM,TAG_SURV(box,a,firstyear),ETA_TAG_SURV(box,a,firstyear),SIGMA_TAG_SURV(box,a,firstyear))
           DO Y=FIRSTYEAR+1,LASTYEAR
             TAG_SURV(box,a,y)=TAG_SURV(box,a,firstyear)
         END DO ; END DO

! {chronic tag shedding or mortality}
         DO A=FIRSTAGE,LASTAGE
           IF(ICALL==0 .and. a==FIRSTAGE .AND. ((PARM_SPECS(I,4)<0.and.PARM_SPECS(I,4)>-1.0) .or. &
             (PARM_SPECS(I,4)>0.0001 .AND. PARM_SPECS(I,4)<0.3))) GOTO 997
           CALL GETPAR(I,K,X,NPARM,TAG_LOSS(box,a,firstyear),ETA_TAG_LOSS(box,a,firstyear),SIGMA_TAG_LOSS(box,a,firstyear))
           DO Y=FIRSTYEAR+1,LASTYEAR
             TAG_LOSS(box,a,y)=TAG_LOSS(box,a,firstyear)
         END DO ; END DO

! {fraction of recaptured tags that are reported}
         DO Y=FIRSTYEAR,LASTYEAR
           IF(ICALL==0.and.Y==FIRSTYEAR.AND.((PARM_SPECS(I,4)<0.and.PARM_SPECS(I,4)>-1.0) .or. &
             (PARM_SPECS(I,4)>0.0001 .AND. PARM_SPECS(I,4)<0.3))) GOTO 998
           CALL GETPAR(I,K,X,NPARM,TAG_REPORT(box,firstage,y),ETA_TAG_report(box,firstage,y),SIGMA_TAG_REPORT(box,firstage,y))
           DO A=FIRSTAGE+1,LASTAGE
             TAG_REPORT(box,a,y)=TAG_REPORT(box,firstage,y)
         END DO ; END DO

! {inflation factor owing to nonmixing in the first year}
         DO Y=FIRSTYEAR,LASTYEAR
           IF(ICALL==0.and.Y==FIRSTYEAR.AND.((PARM_SPECS(I,4)<0.and.PARM_SPECS(I,4)>-1.0) .or. &
             (PARM_SPECS(I,4)>0.0001 .AND. PARM_SPECS(I,4)<0.3))) GOTO 999
           DO A=FIRSTAGE,LASTAGE
             CALL GETPAR(I,K,X,NPARM,TAG_NOMIX(box,a,y),ETA_TAG_NOMIX(box,a,y),SIGMA_TAG_NOMIX(box,a,y))
         END DO ; END DO

! {inflation factor owing to nonmixing in the second year}
         DO Y=FIRSTYEAR,LASTYEAR
           IF(ICALL==0.and.Y==FIRSTYEAR.AND.((PARM_SPECS(I,4)<0.and.PARM_SPECS(I,4)>-1.0) .or. &
             (PARM_SPECS(I,4)>0.0001 .AND. PARM_SPECS(I,4)<0.3))) GOTO 999
           DO A=FIRSTAGE,LASTAGE
             CALL GETPAR(I,K,X,NPARM,TAG_NOMIX2(box,a,y),ETA_TAG_NOMIX2(box,a,y),SIGMA_TAG_NOMIX2(box,a,y))
         END DO ; END DO
...
1     CONTINUE
      ...
      IF(FIRSTCALL==0 .AND. I<PARM_KEY(NPAR+1)+1) THEN
          WRITE(*,'(1x,a46,a50)') 'ERROR: Too many parameters in parameter file: ',infile(2) ; PAUSE; STOP
        ELSEIF(FIRSTCALL==0 .AND. I>PARM_KEY(NPAR+1)+1) THEN
          WRITE(*,'(1x,a45,a50)') 'ERROR: Too few parameters in parameter file: ',infile(2) ; PAUSE; STOP
      ENDIF
      RETURN
991   WRITE(*,*) 'ERROR: The terminal year parameters do not accomodate random walk or random deviation models'; PAUSE ; STOP
992   WRITE(*,*) 'ERROR: You are attempting to link an F-ratio parameter to a terminal year parameter'; PAUSE ; STOP
993   WRITE(*,*) 'ERROR: You are attempting to link a natural mortality parameter to an F-ratio'; PAUSE ;STOP
994   WRITE(*,*) 'ERROR: You are attempting to link a transfer coefficient to a natural mortality rate'; PAUSE ;STOP
995   WRITE(*,*) 'ERROR: You are attempting to link a catchability coefficient to a variance-scaling parameter'; PAUSE ;STOP
996   WRITE(*,*) 'ERROR: You are attempting to link tag survival to a stock-recruit (or catchability) parameter'; PAUSE ;STOP
997   WRITE(*,*) 'ERROR: You are attempting to link a tag shedding parameter to a tag survival fraction'; PAUSE ; STOP
998   WRITE(*,*) 'ERROR: You are attempting to link a tag reporting parameter to a tag shedding parameter'; PAUSE ; STOP
999   WRITE(*,*) 'ERROR: You are attempting to link a tag nonmixing parameter to a tag reporting parameter'; PAUSE ; STOP
1000  WRITE(*,*) 'ERROR: You are attempting to link a variance parameter to a stock-recruit parameter'; PAUSE ; STOP
      END
...
!FLR  WRITE(*,*) 'WRITING TO FILE ',INFILE(4)
...
100        END DO
       ...
       IF(PDF_TAG>0) THEN
        ...
        DO Y=FIRSTYEAR,LASTYEAR
          DO A=FIRSTAGE,LASTAGE ; I = -I ; CALL WRITEPAR(I,K) ; END DO ! negative I tells program to write year and age
        END DO

        WRITE(50,*) '# tag nonmixing factor for second year'
        DO Y=FIRSTYEAR,LASTYEAR
          DO A=FIRSTAGE,LASTAGE ; I = -I ; CALL WRITEPAR(I,K) ; END DO ! negative I tells program to write year and age
        END DO
...
!------------------------------------------------------------------------------
      USE STATISTICS ; USE PARAMETERS
      IMPLICIT NONE
      INTEGER :: I,K,A,Y,g,DROP,SUMDROP,IOLD
...
! tag nonmixing factor for first year'
        DROP=0
        DO Y=FIRSTYEAR,LASTYEAR-1
          DO A=FIRSTAGE,LASTAGE ; CALL COUNTPAR(I,K,IOLD,DROP,SUMDROP) ; END DO
        END DO
        DROP=1 ; DO A=FIRSTAGE,LASTAGE ; CALL COUNTPAR(I,K,IOLD,DROP,SUMDROP) ; END DO
...
!-----------------------------------------------------------------------------
      ...
      REAL (KIND=8) :: Z(Bs,0:As,0:Ys),SURV(Bs,0:As,0:Ys),NTEMP(Bs,0:As,0:Ys),NTIME,TEMP(2), &
      ...
      DO Y=FIRSTYEAR,LASTYEAR
        DO a = FIRSTAGE,LASTAGE
          DO BOX=1,NBOX
            ...
            IF(SEASON_SSB(box)<0) THEN ; NTIME=(1-SURV(box,a,y))/Z(box,a,y) 
              ELSE ; NTIME=SURV(box,a,y)**(SEASON_SSB(box)/12.d0) ; ENDIF
            IF(MODEL_TYPE==2) NTEMP(box,a,y)=N_STOCK(box,a,y)*(1-T(box,a,y))+N_STOCK(oth,a,y)*T(oth,a,y)
            ! spawning stock biomass (recall that ntemp is abundance in each area subsequent to mixing)
            ssb(box,y)=ssb(box,y)+FECUNDITY(box,a)*Ntemp(box,a,y)*WEIGHT_SSB(box,a,y)*NTIME/1000
          ...
          DO 32 Y = FIRSTYEAR,LASTYEAR
            IF(EFFORT_DATA_STORE(g,box,y)>0 .OR. (PDF_EFFORT(g,box)>1 .AND. EFFORT_DATA_STORE(g,box,y)>-9)) THEN
              ...
              DO 33 A=AGE_EFFORT(1,g,box),AGE_EFFORT(2,g,box)
                IF(SEASON_EFFORT(g,box)<0) THEN ; NTIME=(1-SURV(box,a,y))/Z(box,a,y)
                  ELSE ; NTIME=SURV(box,a,y)**(SEASON_EFFORT(g,box)/12.d0)
                ENDIF
                IF(BIO_EFFORT(g,box)==2) THEN  ! index is biomass
                    NTIME=NTIME*WEIGHT_CATCH(g,box,a,y)
                  ELSEIF(BIO_EFFORT(g,box)==3 .OR. BIO_EFFORT(g,box)==5) THEN ! index is in terms of fishing mortality rate
                    SUM_NTIME=SUM_NTIME+NTIME*NTEMP(box,a,y)*SEL_EFFORT(g,box,a,y) ; NTIME=NTIME*F(box,a,y)
                  ELSEIF(BIO_EFFORT(g,box)==4 .OR. BIO_EFFORT(g,box)==6) THEN ! index is in terms of total mortality rate
                    SUM_NTIME=SUM_NTIME+NTIME*NTEMP(box,a,y)*SEL_EFFORT(g,box,a,y) ; NTIME=NTIME*Z(box,a,y)
                ENDIF
                EFFORT(g,box,y)=EFFORT(g,box,y)+SEL_EFFORT(g,box,a,y)*NTIME*NTEMP(box,a,y)
!               hardwire
                IF(BOTH_STOCKS(g,box)>0) THEN ! add abundance of other stock as well
                  IF(SEASON_EFFORT(g,box)<0) THEN ; NTIME=(1.0-SURV(oth,a,y))/Z(oth,a,y)
                    ELSE ; NTIME=SURV(oth,a,y)**(SEASON_EFFORT(g,box)/12.d0)
                  ENDIF
                  IF(BIO_EFFORT(g,box)==2) NTIME=NTIME*WEIGHT_SSB(oth,a,y) ! this is a patch, can't input WEIGHT_CATCH(g,oth...
                  EFFORT(g,box,y)=EFFORT(g,box,y)+SEL_EFFORT(g,box,a,y)*NTIME*NTEMP(oth,a,y)
                ENDIF
33            CONTINUE
              IF(BIO_EFFORT(g,box)>2) THEN ! F or Z indices
...
32        CONTINUE
          IF(BIO_EFFORT(g,box)==3 .or. BIO_EFFORT(g,box)==4) THEN
            ...
            END DO
          ENDIF
          IF(OPTION(4)<=0 .and. PDF_EFFORT(g,box)/=12 ) CALL GET_Q_BY_MLE ! Estimate q by mle
          DO 36 Y=FIRSTYEAR,LASTYEAR
            IF(PDF_EFFORT(g,box)==12) Q_EFFORT(g,box,y)=1
            IF(EFFORT_DATA_STORE(g,box,y)>0 .OR. (PDF_EFFORT(g,box)>1 .AND. EFFORT_DATA_STORE(g,box,y)>-9)) THEN
              EFFORT(g,box,y)=EFFORT(g,box,y)*Q_EFFORT(g,box,y)
              IF(PDF_EFFORT(g,box)==1.AND.BIO_EFFORT(g,box)>2) EFFORT(g,box,y)=EFFORT(g,box,y)+0.00001
            ENDIF
36        CONTINUE
...
!-------------------------------------------------------------------
      USE STATISTICS  ; USE DATUM, ONLY : SEL_TYPE, PSEL, EFFORT_DATA, PDF_EFFORT 
      IMPLICIT NONE
      ...
      IF(SEL_TYPE(gear,box)<=3) THEN ! Year-specific selectivities-- Powers and Restrepo (1992)
          DO Y=FIRSTYEAR,LASTYEAR
            PMAX=-1.D0
            DO AGE=AGE_EFFORT(1,gear,box),AGE_EFFORT(2,gear,box)
        ...
        ELSE ! constant selectivities-- Butterworth and Geromont (1999)
          PMAX=-1.D0 ; FBEST=0
          DO 2 AGE=AGE_EFFORT(1,gear,box),AGE_EFFORT(2,gear,box)
            DO 9 Y=FIRSTYEAR,LASTYEAR
              IF(EFFORT_DATA(gear,box,y).GT.0 .or. (EFFORT_DATA(gear,box,y)>-9 .and. PDF_EFFORT(gear,box)>1)) THEN !
                FBEST(age)=FBEST(age)+F(box,age,y)*PSEL(gear,box,age,y)
              ENDIF
9           CONTINUE
2           PMAX=MAX(pmax,fbest(age))
          DO 11 Y=FIRSTYEAR,LASTYEAR
            IF(EFFORT_DATA(gear,box,y).GT.0 .or. (EFFORT_DATA(gear,box,y)>-9 .and. PDF_EFFORT(gear,box)>1)) THEN !
              DO 10 AGE=AGE_EFFORT(1,gear,box),AGE_EFFORT(2,gear,box)
10              SEL_EFFORT(gear,box,age,y)=FBEST(age)/PMAX
            ENDIF
11        CONTINUE
      ENDIF
...
!------------------------------------------------------------------------------
! Estimates the distribution of tag returns between boxes and computes
! the diagnostic ratios of Punt and Butterworth.
      USE STATISTICS ; USE PARAMETERS ; USE DATUM , ONLY : NEWTAGS,TAG_TIME,FISH_TIME
      IMPLICIT NONE
      INTEGER OTH,YREL,A
      REAL*8 BOTTOM,z1,z2,F1,S1,S2,NEWTAGS_BY_STOCK(2,2,As,Ys),t1,t2

! \\\\\\\\\\\\\\\\\\\\\\\\\\\              
! table of variables
! ///////////////////////////
! NEWTAGS(box,age,year) =   number of fish tagged by age, year, and area (box)
! N_TAGS(i,j,a,y1,y2)   =   number of fish originally tagged in box i in year y1 at age a now alive at beginning of year y2
!                           in area j (diffusion model) or belonging to stock j (overlap model)
! R(i,j,a,y1,y2)       =   number of fish originally tagged in box i at age a in year y1
!                          that were recaptured in box j in year y2
! TAG_TIME(box,a,yrel) =   fraction of first year tag cohort is exposed to fishing (1 - fraction of year when tag cohort released)

! \\\\\\\\\\\\\\\\\\\\\\\\\\\
! fill in estimated tag abundance matrix
! ///////////////////////////      
      
      N_TAGS=0 ; R=0
      IF(MODEL_TYPE==2) THEN ! {partition fish tagged in each area by stock, assuming the same ratios as the untagged population}
        NEWTAGS_BY_STOCK=0
        DO 10 BOX=1,2
          IF(BOX.EQ.1) THEN ; oth=2 ; ELSE ; oth=1 ; ENDIF
          DO 10 AGE=FIRSTAGE,LASTAGE
            DO 10 YEAR=FIRSTYEAR,LASTYEAR
              BOTTOM=N_STOCK(box,age,year)*(1-T(box,age,year))+N_STOCK(oth,age,year)*T(oth,age,year)
              IF(BOTTOM>0) THEN
                  NEWTAGS_BY_STOCK(box,box,age,year)=NEWTAGS(box,age,year)*N_STOCK(box,age,year)*(1-T(box,age,year))/BOTTOM
                  NEWTAGS_BY_STOCK(box,oth,age,year)=NEWTAGS(box,age,year)*N_STOCK(oth,age,year)*T(oth,age,year)/BOTTOM
                ELSE
                  NEWTAGS_BY_STOCK(box,box,age,year)=NEWTAGS(box,age,year)
                  NEWTAGS_BY_STOCK(box,oth,age,year)=0
              ENDIF
10      CONTINUE      
      ENDIF

!     {compute tagging statistics}
!     here tag releases are assumed to occur at some point during the year
      DO 200 Yrel = FIRSTYEAR,LASTYEAR ! year of release
       DO 200 A=FIRSTAGE,LASTAGE ! age at release
         DO 200 Year =Yrel,LASTYEAR ! year of recapture
          DO 210 Box=1,NBOX ! area of release
            IF(NEWTAGS(box,a,yrel)<1) CYCLE
            IF(BOX.EQ.1) THEN ; OTH=2 ; ELSE ; OTH=1 ; ENDIF
            AGE=A+Year-yrel ; IF(AGE>PLUSGROUP) AGE=PLUSGROUP ! age at recapture
            T1=T(box,age,year) ; T2=T(oth,age,year)
            IF(MODEL_TYPE==1) THEN ! {diffusion model}
               IF(YEAR==YREL) THEN
!                  assume tags don't redistribute until next year
                   Z1=(M(box,age,year)+TAG_LOSS(box,a,yrel))*TAG_TIME(box,a,yrel)+ &
                      TAG_NOMIX(box,age,year)*F(box,age,year)*FISH_TIME(box,a,yrel)
                   S1=EXP(-Z1)
                   R(box,box,a,yrel,yrel)=TAG_REPORT(box,a,yrel)*TAG_NOMIX(box,age,yrel)*F(box,age,yrel)*FISH_TIME(box,a,yrel)/z1*&
                                          NEWTAGS(box,a,yrel)*TAG_SURV(box,a,yrel)*(1.0-S1)
                   N_TAGS(box,box,a,yrel,yrel+1)=NEWTAGS(box,a,yrel)*TAG_SURV(box,a,yrel)*S1
                   N_TAGS(box,oth,a,yrel,yrel+1)=0
                 ELSE
                   IF(YEAR==YREL+1) THEN ; F1=TAG_NOMIX2(box,age,year)*F(box,age,year) ; ELSE ; F1=F(box,age,year) ; ENDIF
                   Z1=M(box,age,year)+F1+TAG_LOSS(box,a,yrel) ; S1=DEXP(-Z1)
                   Z2=M(oth,age,year)+F(oth,age,year)+TAG_LOSS(box,a,yrel) ; S2=DEXP(-Z2) ! assume fish that crosses is fully mixed
!                  {compute the tag abundance after mixing. Assume tags are released after mixing}
                   N_TAGS(box,box,a,yrel,year+1)=N_TAGS(box,box,a,yrel,year)*(1.0-T1)+N_TAGS(box,oth,a,yrel,year)*T2
                   N_TAGS(box,oth,a,yrel,year+1)=N_TAGS(box,box,a,yrel,year)*T1+N_TAGS(box,oth,a,yrel,year)*(1.0-T2)
!                  {compute the recaptures (note that N_TAGS(i,j,a,year+1) at this point still refers to this year's abundance)}
                   R(box,box,a,yrel,year)=TAG_REPORT(box,age,year)*F1/z1*N_TAGS(box,box,a,yrel,year+1)*(1.0-S1)
                   R(box,oth,a,yrel,year)=TAG_REPORT(oth,age,year)*F(oth,age,year)/Z2*N_TAGS(box,oth,a,yrel,year+1)*(1.0-S2)
!                  {compute the tag abundance at beginning of next year (but prior to mixing)}
                   N_TAGS(box,box,a,yrel,year+1)=N_TAGS(box,box,a,yrel,year+1)*S1
                   N_TAGS(box,oth,a,yrel,year+1)=N_TAGS(box,oth,a,yrel,year+1)*S2
               ENDIF
             ELSE ! {Overlap model}
               IF(YEAR==YREL) THEN
!                  assume tags don't redistribute until next year
                   ...
                   R(box,box,a,yrel,yrel)=TAG_REPORT(box,a,yrel)*TAG_NOMIX(box,age,yrel)*F(box,age,yrel)*FISH_TIME(box,a,yrel)/z1* &
                                          NEWTAGS(box,a,yrel)*TAG_SURV(box,a,yrel)*(1.0-S1)
                   N_TAGS(box,box,a,yrel,yrel+1)=NEWTAGS_BY_STOCK(box,box,a,yrel)*TAG_SURV(box,a,yrel)*S1
                   N_TAGS(box,oth,a,yrel,yrel+1)=NEWTAGS_BY_STOCK(box,oth,a,yrel)*TAG_SURV(box,a,yrel)*S1
                 ELSE
                   IF(YEAR==YREL+1) THEN ; F1=TAG_NOMIX2(box,age,year)*F(box,age,year) ; ELSE ; F1=F(box,age,year) ; ENDIF
                   Z1=M(box,age,year)+F1+TAG_LOSS(box,a,yrel) ; S1=DEXP(-Z1)
                   Z2=M(oth,age,year)+F(oth,age,year)+TAG_LOSS(box,a,yrel) ; S2=DEXP(-Z2) ! assume fish that crosses is fully mixed
!                  {compute the tag abundance at the beginning of next year assuming mid-year releases}
                   N_TAGS(box,box,a,yrel,year+1)=N_TAGS(box,box,a,yrel,year)*(T1*S2+(1-T1)*S1)
                   N_TAGS(box,oth,a,yrel,year+1)=N_TAGS(box,oth,a,yrel,year)*(T2*S1+(1-T2)*S2)
!                  {compute the recaptures (note that N_TAGS(i,j,age+1,year+1) still refers to the abundance during age,year)}
                   R(box,box,a,yrel,year)=TAG_REPORT(box,age,year)*F1/z1*(1.0-S1)*(N_TAGS(box,box,a,yrel,year)* &
                                          (1.0-T1)+N_TAGS(box,oth,a,yrel,year)*T2)
                   R(box,oth,a,yrel,year)=TAG_REPORT(oth,age,year)*F(oth,age,year)/z2*(1.0-S2)*(N_TAGS(box,oth,a,yrel,year)* &
                                          (1.0-T2)+N_TAGS(box,box,a,yrel,year)*T1)
               ENDIF
            ENDIF
210       CONTINUE
200   CONTINUE
...
!------------------------------------------------------------------------------
      ...
      IMPLICIT NONE
      INTEGER G,Y,A,NPARM,I,B_REL,Y_REL,A_REL,FIRST_SSB
      ...
      DUMMY=0
      IF(ICALL==0) N_DATA=0
      IF(NPARM==0) THEN ; EFFORT=EFFORT_DATA ; R=RDATA ; GOTO 99 ; END IF ! Deviance calculations require likelihood of perfect fit
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!      
         DO a=FIRSTAGE,LASTAGE
          IF(SIGMA_TAG_SURV(box,a,firstyear)>0) LIKE_TAG_SURV(box)=LIKE_TAG_SURV(box) &
            -0.5*(ETA_TAG_SURV(box,a,firstyear)/SIGMA_TAG_SURV(box,a,firstyear))**2.0 - LOG(SIGMA_TAG_SURV(box,a,firstyear))
         END DO
         POSTERIORS = POSTERIORS + LIKE_TAG_SURV(box)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Compute posterior for tag shedding deviations
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!      
        DO a=FIRSTAGE,LASTAGE
         IF(SIGMA_Tag_loss(box,a,firstyear)>0) LIKE_Tag_loss(box)=LIKE_Tag_loss(box) &
           -0.5*(ETA_Tag_loss(box,a,firstyear)/SIGMA_Tag_loss(box,a,firstyear))**2.0 - LOG(SIGMA_TAG_LOSS(box,a,firstyear))
        END DO
        POSTERIORS = POSTERIORS + LIKE_Tag_loss(box)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Compute posterior for tag reporting deviations
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!      
        DO y=FIRSTYEAR,LASTYEAR
         IF(SIGMA_Tag_report(box,firstage,y)>0) LIKE_Tag_report(box)=LIKE_Tag_report(box) &
           -0.5*(ETA_Tag_report(box,firstage,y)/SIGMA_Tag_report(box,firstage,y))**2.0 - LOG(SIGMA_TAG_REPORT(box,firstage,y))
        END DO
        POSTERIORS = POSTERIORS + LIKE_Tag_report(box)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Compute posterior for tag-nonmixing-in-first-year deviations
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!      
        DO a=FIRSTAGE,LASTAGE
         IF(SIGMA_Tag_nomix(box,a,firstyear)>0) LIKE_Tag_nomix(box)=LIKE_Tag_nomix(box) &
           -0.5*(ETA_Tag_nomix(box,a,firstyear)/SIGMA_Tag_nomix(box,a,firstyear))**2.0 - LOG(SIGMA_TAG_nomix(box,a,firstyear))
        END DO
        POSTERIORS = POSTERIORS + LIKE_Tag_nomix(box)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Compute posterior for tag-nonmixing-in-second-year deviations
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!      
        DO a=FIRSTAGE,LASTAGE
         IF(SIGMA_Tag_nomix2(box,a,firstyear)>0) LIKE_Tag_nomix(box)=LIKE_Tag_nomix(box) &
           -0.5*(ETA_Tag_nomix2(box,a,firstyear)/SIGMA_Tag_nomix2(box,a,firstyear))**2.0 - LOG(SIGMA_TAG_nomix2(box,a,firstyear))
        END DO
        POSTERIORS = POSTERIORS + LIKE_Tag_nomix(box)

      ENDIF ! tagging parameters
        ...
        DO B_REL=1,NBOX
          DO Y_REL=FIRSTYEAR,LASTYEAR
            DO A_REL=FIRSTAGE,LASTAGE
              IF(NEWTAGS(b_rel,a_rel,y_rel)>0) THEN
                SUMPROB=0
                DO Y=Y_REL,LASTYEAR
                  DO BOX=1,NBOX
                    PRED=R(b_rel,box,a_rel,y_rel,y) ; SUMPROB=SUMPROB+PRED
                    IF(PDF_TAG==3) PRED=PRED/NEWTAGS(b_rel,a_rel,y_rel)
                    CALL FORMULAE(RDATA(b_rel,box,a_rel,y_rel,y),pred,SIGMA_TAG(b_rel,a_rel,y_rel),DUMMY,PDF_TAG,ANSWER,CONSTANTS)
                    IF(PDF_TAG==3) ANSWER=ANSWER/SIGMA_TAG(b_rel,a_rel,y_rel)**2.0
                    LIKE_TAG(b_rel)=LIKE_TAG(b_rel)+ANSWER
                    IF(ICALL==0) N_DATA=N_DATA+1
                  END DO
                END DO
                PRED=NEWTAGS(b_rel,a_rel,y_rel)-SUMPROB
                IF(PDF_TAG==3) PRED=PRED/NEWTAGS(b_rel,a_rel,y_rel)
                OBS=NEWTAGS(b_rel,a_rel,y_rel)-SUMRDATA(b_rel,a_rel,y_rel)
                CALL FORMULAE(OBS,pred,SIGMA_TAG(b_rel,a_rel,y_rel),DUMMY,PDF_TAG,ANSWER,CONSTANTS)
                IF(PDF_TAG==3) THEN
                  IF(CONSTANTS=='Y') ANSWER=ANSWER+LOGFACTORIAL(NEWTAGS(b_rel,a_rel,y_rel)-1)
                  ANSWER=ANSWER/SIGMA_TAG(b_rel,a_rel,y_rel)**2.0
                ENDIF
                LIKE_TAG(b_rel)=LIKE_TAG(b_rel)+ANSWER
                IF(ICALL==0) N_DATA=N_DATA+1
              ENDIF
            END DO ! A_REL
          END DO ! Y_REL
          LIKELIHOODS = LIKELIHOODS + LIKE_TAG(b_rel)
        END DO ! B_REL
      ENDIF
...
!-----------------------------------------------------------------
      ...
      if(xvec(1).gt.10) then ; xvec(1)=10 ; elseif(xvec(1).lt.0.00001) then ; xvec(1)=0.00001 ; endif
      if(xvec(2).gt.10) then ; xvec(2)=10 ; elseif(xvec(2).lt.0.00001) then ; xvec(2)=0.00001 ; endif
      X=XVEC(1) ; Y=XVEC(2)
      IF(LOOP<=1) THEN
        ...
        c4=c*v*(ti-2*u1)-d*u1*tf ; a5=m1*(a*u*(ti-2*u1)-b*t1*tf) ; a6=m1*(2*x*tu1aub-a*u1*tf+b*v*(ti-2*u1))
        IF(LOOP<1) THEN ; XVEC(1)=K*(M1+0.05)/(EXP(0.05+M1)-1)/(a+b); XVEC(2)=l*(M1+0.05)/(EXP(0.05+M1)-1)/(c+d)
          XVEC(1)= MIN(1.0,MAX(XVEC(1),0.01D0)) ; XVEC(2)= MIN(1.0,MAX(XVEC(2),0.01D0)) ; RETURN
        ENDIF
      ENDIF
...
!-----------------------------------------------------------------
      ...
      IMPLICIT NONE
      INTEGER LOOP,NP
      REAL (KIND = 8) ::  X,Y,F,G,FX,FY,GX,GY,a,b,c,d,K,J,O,P,R,S,N1,N2,XVEC(np),fvec(np),df(np,np)
      ...
      IF(LOOP<=1) THEN
        ...
        IF(LOOP<1) THEN  
          ...
          XVEC(1)=k*o/(1-exp(-o))/(n1*a+n2*b) ; XVEC(2)=j*p/(1-exp(-p))/(n2*d+n1*c) ; RETURN
        ENDIF
      ENDIF
!      x=min(10.0,max(x,0.00000001)) ; y=min(10.0,max(y,0.00000001))
      ...
      F = (c*EXP(-y-p)+a*EXP(-x-o))*((d*k*(x+o)/x/(1-EXP(-x-o))-b*j*(y+p)/y/(1-EXP(-y-p)))/(a*d-b*c))
      
      G = (b*EXP(-x-o)+d*EXP(-y-p))*((a*j*(y+p)/y/(1-EXP(-y-p))-c*k*(x+o)/x/(1-EXP(-x-o)))/(a*d-b*c))

      FX = EXP(-x-y-o-p)*(c*d*k*o*y*EXP(3*x+3*o)*(EXP(y+p)-1)-EXP(2*x+2*o &
     )*(a*EXP(2*y+2*p)*(x**2*(y*(b*j-d*k)+b*j*p)-d*k*o*x*y-d*k*o*y)+d*      &
     k*y*EXP(y+p)*(x**2*(a-c)+o*x*(a-c)+o*(a+c))+c*d*k*y*(x**2+o*x-o))      &
     +a*EXP(x+y+o+p)*(EXP(y+p)*(2*b*j*x**2*(y+p)-d*k*o*y)+d*k*o*y)-a*      &
     b*j*x**2*EXP(2*y+2*p)*(y+p))/(x**2*y*(EXP(x+o)-1)**2*(EXP(y+p)-      &
     1)*(b*c-a*d))

      FY = EXP(-x-y-o-p)*(c*EXP(2*x+2*o)*(EXP(2*y+2*p)*(x*(y**2*(b*j-d*k)+  &
     b*j*p*y+b*j*p)-d*k*o*y**2)+EXP(y+p)*(x*(2*d*k*y**2-b*j*p)+2*d*k*o       &
     *y**2)-d*k*y**2*(x+o))+b*j*x*EXP(x+y+o+p)*(a*p*EXP(2*y+2*p)+EXP       &
     (y+p)*(y**2*(a-c)+p*y*(a-c)-p*(a+c))+c*p)-a*b*j*x*EXP(2*y+2*p)*(p       &
     *EXP(y+p)+y**2+p*y-p))/(x*y**2*(EXP(x+o)-1)*(EXP(y+p)-1)**2*(a*       &
     d-b*c))

      GX = EXP(-x-y-o-p)*(c*d*k*o*y*EXP(3*x+3*o)*(EXP(y+p)-1)-EXP(2*x+2*o &
     )*(b*EXP(2*y+2*p)*(x**2*(y*(a*j-c*k)+a*j*p)-c*k*o*x*y-c*k*o*y)+c*       &
     k*y*EXP(y+p)*(x**2*(b-d)+o*x*(b-d)+o*(b+d))+c*d*k*y*(x**2+o*x-o))       &
     +b*EXP(x+y+o+p)*(EXP(y+p)*(2*a*j*x**2*(y+p)-c*k*o*y)+c*k*o*y)-a*       &
     b*j*x**2*EXP(2*y+2*p)*(y+p))/(x**2*y*(EXP(x+o)-1)**2*(EXP(y+p)-       &
     1)*(a*d-b*c))

      GY = EXP(-x-y-o-p)*(d*EXP(2*x+2*o)*(EXP(2*y+2*p)*(x*(y**2*(a*j-c*k)+  &
     a*j*p*y+a*j*p)-c*k*o*y**2)+EXP(y+p)*(x*(2*c*k*y**2-a*j*p)+2*c*k*o       &
     *y**2)-c*k*y**2*(x+o))+a*j*x*EXP(x+y+o+p)*(b*p*EXP(2*y+2*p)+EXP       &
     (y+p)*(y**2*(b-d)+p*y*(b-d)-p*(b+d))+d*p)-a*b*j*x*EXP(2*y+2*p)*(p       &
     *EXP(y+p)+y**2+p*y-p))/(x*y**2*(EXP(x+o)-1)*(EXP(y+p)-1)**2*(b*       &
     c-a*d))
      ...
      F = F + (c*EXP(-s*y-p)+a*EXP(-r*x-o))*((d*k*(r*x+o)/(r*x)/(1-EXP(-r*x-o))-b*j*(s*y+p)/(s*y)/(1-EXP(-s*y-p)))/(a*d-b*c)) 

      G = G + (b*EXP(-r*x-o)+d*EXP(-s*y-p))*((a*j*(s*y+p)/(s*y)/(1-EXP(-s*y-p))-c*k*(r*x+o)/(r*x)/(1-EXP(-r*x-o)))/(a*d-b*c)) 
     
     FX = FX + EXP(-r*x-s*y-o-p)*(c*d*k*o*s*y*EXP(3*r*x+3*o)*(EXP(s*y+p)-1)-   &
     EXP(2*r*x+2*o)*(a*EXP(2*s*y+2*p)*(r**2*x**2*(s*y*(b*j-d*k)+b*j*p)-             &
     d*k*o*r*s*x*y-d*k*o*s*y)+d*k*s*y*EXP(s*y+p)*(r**2*x**2*(a-c)+o*r*             &
     x*(a-c)+o*(a+c))+c*d*k*s*y*(r**2*x**2+o*r*x-o))+a*EXP(r*x+s*y+o+p             &
     )*(EXP(s*y+p)*(2*b*j*r**2*x**2*(s*y+p)-d*k*o*s*y)+d*k*o*s*y)-a*b*             &
     j*r**2*x**2*EXP(2*s*y+2*p)*(s*y+p))/(r*s*x**2*y*(EXP(r*x+o)-1)**             &
     2*(EXP(s*y+p)-1)*(b*c-a*d))

      FY = FY + EXP(-r*x-s*y-o-p)*(c*EXP(2*r*x+2*o)*(EXP(2*s*y+2*p)*(r*x*(s**2*   &
     y**2*(b*j-d*k)+b*j*p*s*y+b*j*p)-d*k*o*s**2*y**2)+EXP(s*y+p)*(r*x*             &
     (2*d*k*s**2*y**2-b*j*p)+2*d*k*o*s**2*y**2)-d*k*s**2*y**2*(r*x+o))+             &
     b*j*r*x*EXP(r*x+s*y+o+p)*(a*p*EXP(2*s*y+2*p)+EXP(s*y+p)*(s**2*y             &
     **2*(a-c)+p*s*y*(a-c)-p*(a+c))+c*p)-a*b*j*r*x*EXP(2*s*y+2*p)*(p*             &
     EXP(s*y+p)+s**2*y**2+p*s*y-p))/(r*s*x*y**2*(EXP(r*x+o)-1)*(EXP(s             &
     *y+p)-1)**2*(a*d-b*c))

      GX = GX + EXP(-r*x-s*y-o-p)*(c*d*k*o*s*y*EXP(3*r*x+3*o)*(EXP(s*y+p)-1)-  &
     EXP(2*r*x+2*o)*(b*EXP(2*s*y+2*p)*(r**2*x**2*(s*y*(a*j-c*k)+a*j*p)-             &
     c*k*o*r*s*x*y-c*k*o*s*y)+c*k*s*y*EXP(s*y+p)*(r**2*x**2*(b-d)+o*r*             &
     x*(b-d)+o*(b+d))+c*d*k*s*y*(r**2*x**2+o*r*x-o))+b*EXP(r*x+s*y+o+p             &
     )*(EXP(s*y+p)*(2*a*j*r**2*x**2*(s*y+p)-c*k*o*s*y)+c*k*o*s*y)-a*b*             &
     j*r**2*x**2*EXP(2*s*y+2*p)*(s*y+p))/(r*s*x**2*y*(EXP(r*x+o)-1)**             &
     2*(EXP(s*y+p)-1)*(a*d-b*c))

      GY = GY + EXP(-r*x-s*y-o-p)*(d*EXP(2*r*x+2*o)*(EXP(2*s*y+2*p)*(r*x*(s**2*  &
     y**2*(a*j-c*k)+a*j*p*s*y+a*j*p)-c*k*o*s**2*y**2)+EXP(s*y+p)*(r*x*            &
     (2*c*k*s**2*y**2-a*j*p)+2*c*k*o*s**2*y**2)-c*k*s**2*y**2*(r*x+o))+            &
     a*j*r*x*EXP(r*x+s*y+o+p)*(b*p*EXP(2*s*y+2*p)+EXP(s*y+p)*(s**2*y            &
     **2*(b-d)+p*s*y*(b-d)-p*(b+d))+d*p)-a*b*j*r*x*EXP(2*s*y+2*p)*(p*            &
     EXP(s*y+p)+s**2*y**2+p*s*y-p))/(r*s*x*y**2*(EXP(r*x+o)-1)*(EXP(s            &
     *y+p)-1)**2*(b*c-a*d))
...
!      if(loop>0) write(*,*) loop,'catch',x
      IF(LOOP<=1) THEN
       ...
       IF(LOOP<1) THEN ; X=(m1+0.05)*K/a/(EXP(m1+0.05)-1) ; XVEC(1)= MIN(3.0,MAX(x,0.00000001D0)) ; RETURN ; ENDIF
      ENDIF
...
!      if(loop.gt.0) write(*,*) loop,'plus',x
      IF(LOOP<=1) THEN
       ...
       IF(LOOP<1) THEN ; X=O/(K*(EXP(O+0.02)-1)/(A+B+1)) ; XVEC(1)=MIN(3.0,MAX(x,0.0001D0)) ; RETURN ; ENDIF
      ENDIF
...
!------------------------------------------------------------------------------
      USE STATISTICS ; USE PARAMETERS ; USE DATUM , ONLY : NEWTAGS
      IMPLICIT NONE
      INTEGER o,b
...
!     {the recruitment to each stock is set to that of the corresponding area}          
      DO b=1,2
        DO YEAR=FIRSTYEAR,LASTYEAR
          N_TAGS(b,b,firstage,year,1)=N_AREA(b,firstage,year)
      END DO ; END DO

      DO 200 YEAR = FIRSTYEAR,LASTYEAR
        DO 210 b=1,2
         IF(b.EQ.1) THEN ; o=2 ; ELSE ; o=1 ; ENDIF
         DO 220 AGE=FIRSTAGE,LASTAGE
!          {abundance after mixing} 
           N_TAGS(b,b,age+1,year+1,1)=N_TAGS(b,b,age,year,1)*(1-T(b,age,year))+N_TAGS(b,o,age,year,1)*T(o,age,year)
           N_TAGS(b,o,age+1,year+1,1)=N_TAGS(b,b,age,year,1)*T(b,age,year)+N_TAGS(b,o,age,year,1)* &
                                             (1-T(o,age,year))
           Z1=M(b,age,year)+F(b,age,year) ; S1=DEXP(-Z1)
           Z2=M(o,age,year)+F(o,age,year) ; S2=DEXP(-Z2)

!          {compute the tag abundance at beginning of next year (but prior to mixing)}
           N_TAGS(b,b,age+1,year+1,1)=N_TAGS(b,b,age+1,year+1,1)*S1
           N_TAGS(b,o,age+1,year+1,1)=N_TAGS(b,o,age+1,year+1,1)*S2
220      CONTINUE
         IF(PLUSGROUP>1) THEN  
!         {accumulate older fish in the last age group}          
          N_TAGS(b,b,lastage,year+1,1)=N_TAGS(b,b,lastage,year+1,1)+N_TAGS(b,b,lastage+1,year+1,1)
          N_TAGS(b,o,lastage,year+1,1)=N_TAGS(b,o,lastage,year+1,1)+N_TAGS(b,o,lastage+1,year+1,1)
         ENDIF  
210     CONTINUE
200   CONTINUE
      DO YEAR=FIRSTYEAR,LASTYEAR+1
        DO b=1,2
         ...
         DO AGE=FIRSTAGE,LASTAGE
           N_STOCK(b,age,year)=N_TAGS(b,b,age,year,1)+N_TAGS(b,o,age,year,1)
      END DO ; END DO ; END DO
      RETURN ; END
...
!----------------------------------------------------------------------------
      ...
      NBOOTS=ABS(OPTION(3)) ; NAGES=LASTAGE-FIRSTAGE+1
      ...
      IF(PDF_STOCKRECRUIT.NE.0) THEN
        ...
        OPEN(70,FILE='SR.OUT',ACCESS='DIRECT',STATUS='UNKNOWN',RECL=IRECL)
      ENDIF
        ...
        DO y = 1,NE(g,b)
          IF(NPI(g,b)<NE(g,b)) THEN
            ...
            ELSE
              ...
              WRITE(*,*) '       than data points (',ne(g,b),')' ; PAUSE; STOP
          ENDIF
        END DO ! Y
       END DO ! G
      END DO ! B
...
201           sI(g,b,y)=SNGL(EFFORT_DATA(g,b,y)) 
            DO 200 a = Firstage,Lastage
              sT(b,a,y)=SNGL(T(b,a,y)) ; sM(b,a,y)=SNGL(M(b,a,y)) ; sCAA(b,a,y)=SNGL(CATCH_DATA(b,a,y)) ; sFAA(b,a,y)=SNGL(F(b,a,y))
              ...
              IF(Y==lastyear) then ; sTERM(b,a)=SNGL(SEL_TERMINAL(b,a,1)) ; 
...
200         CONTINUE  
210     CONTINUE        
        WRITE(62,REC=LOOP) (((sNAA(b,a,y),Y=firstyear,lastyear),A=firstage,lastage),b=1,NBOX)
        WRITE(63,REC=LOOP) (((sFAA(b,a,y),Y=firstyear,lastyear),A=firstage,lastage),b=1,NBOX)
        WRITE(64,REC=LOOP) (((sCAA(b,a,y),Y=firstyear,lastyear),A=firstage,lastage),b=1,NBOX)
        WRITE(65,REC=LOOP) (((sM(b,a,y),Y=firstyear,lastyear),A=firstage,lastage),b=1,NBOX)
        IF(nbox>1) WRITE(66,REC=LOOP) (((sT(b,a,y),Y=firstyear,lastyear),A=firstage,lastage),b=1,NBOX)
        WRITE(67,REC=LOOP) ((sI(g,1,y),Y=firstyear,lastyear),g=1,NGEARS(1)),((sI(g,2,y),Y=firstyear,lastyear),g=1,NGEARS(2))
        WRITE(68,REC=LOOP) ((sTerm(b,a),A=firstage,lastage),b=1,NBOX)
        IF(PDF_STOCKRECRUIT.NE.0) THEN
          IRECL=(6+NYEARS)*NBOX*4
          WRITE(70,REC=LOOP) ((sR(b,Y),Y=1,5+lastyear),b=1,NBOX)
        ENDIF

! Don't use MLE as though it were a bootstrap for bias corrections
        IF(LOOP==1) GOTO 100

! {report bad records to ASC-II file 'BAD.OUT}
...
51      CONTINUE
        ...
        ENDIF
        IF(OBJ.GT.PARM_EST_STORE(PARM_KEY(0)+1) .AND. (ABS(OBJ)/ABS(PARM_EST_STORE(PARM_KEY(0)+1)))>10) THEN ;
            write(69,*) LOOP,LOOPKILL,' very poor fit-- objective function was ',obj
            GOTO 100 ! Ignore really bad fits when computing confidence intervals and bias correction
        ENDIF
        COUNT=COUNT+1
...
!----------------------------------------------------------------------------
      REAL*8 FUNCTION BOOTERROR(ind,iar,Y,E,E_s,NE,ISEED,GS,BS,YS)
!----------------------------------------------------------------------------
