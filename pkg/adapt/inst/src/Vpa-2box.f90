!
! Last change:  18 Mar 2004 21:07
! program VPA-2BOX.F90 , VERSION 3.01
! A sequential population analysis tool
! programmed by Clay E. Porch in FORTRAN 90
! 11/1/2000
!
! changes for 3.01: 1) use bisection routine for 1 dimensional problems rather than Newton's method
!                   2) correct formula for chi-square discrepancy of lognormal deviates
!                   3) change AIC and BIC formulae from deviance-based to full-likelihood expressions to
!                      allow comparison of models where the variances are estimatedorrect formula for chi-square discrepancy of lognormal deviates
!                   4) put in error message to prevent use of overlap model when estimating terminal parameters on N
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      MODULE DIMS
        INTEGER :: As, Ys, Gs, Bs  ! maximum age (or max. # length bins), maximum fisheries or surveys, maximum year blocks
        PARAMETER (As=20 , Ys=110, Gs=20 , Bs=2)
      END MODULE
      ...
      MODULE PARAMETERS
        ...
        CHARACTER (LEN=75) :: INFILE(0:10),CONFILE
      ...
      MODULE DATUM
        ...
        REAL (KIND=8) :: CATCH_DATA(Bs,0:As,0:Ys),EFFORT_DATA(Gs,Bs,0:Ys),PSEL(Gs,Bs,0:As,0:Ys),  &
                         ...
                         SIGMA_TAG_REPORT(Bs,0:As,0:Ys),SIGMA_TAG_SURV(Bs,0:As,0:Ys),SIGMA_TAG_LOSS(Bs,0:As,0:Ys), &
                         SIGMA_Q(Gs,Bs,0:Ys),INDEX_MEAN(Gs,Bs),WEIGHT_CATCH(Gs,Bs,0:As,0:Ys),FECUNDITY(Bs,0:As), &
                         WEIGHT_SSB(Bs,0:As,0:Ys),RDATA(Bs,Bs,0:As,0:Ys,0:Ys),NEWTAGS(Bs,0:As,0:Ys),SIGMA_TAG(Bs,0:As,0:Ys), &
                         EFFORT_DATA_STORE(Gs,Bs,0:Ys),  SUMRDATA(Bs,0:As,0:Ys),TAG_TIME(Bs,0:As,0:Ys),SIGMA_TAG_NOMIX(Bs,0:As,0:Ys), &
                         SIGMA_TAG_NOMIX2(Bs,0:As,0:Ys),FISH_TIME(Bs,0:As,0:Ys)
      ...
      MODULE STATISTICS
        USE DIMS
        REAL (KIND=8) :: N_STOCK(Bs,0:As,0:Ys),N_AREA(Bs,0:As,0:Ys),F(Bs,0:As,0:Ys),M(Bs,0:As,0:Ys),T(Bs,0:As,0:Ys), &
                         EFFORT(Gs,Bs,0:Ys),SEL_EFFORT(Gs,Bs,0:As,0:Ys),Q_EFFORT(Gs,Bs,0:Ys), & 
                         PLUSAGE(Bs,0:Ys),STOCK_RECRUIT(Bs,5),SSB(Bs,0:Ys),RECRUITS(Bs,0:Ys),SCALE_VARIANCE(Gs,Bs), &
                         SEL_TERMINAL(Bs,0:As,2),F_RATIO(Bs,0:Ys),TAG_LOSS(Bs,0:As,0:Ys),TAG_REPORT(Bs,0:As,0:Ys),TAG_SURV(Bs,0:As,0:Ys), &
                         N_TAGS(Bs,Bs,0:As,0:Ys,0:Ys),R(Bs,Bs,0:As,0:Ys,0:Ys),ETA_Q(Gs,Bs,0:Ys),ETA_F(Bs,0:Ys),ETA_SR(Bs,0:Ys)=0, &
                         ETA_T(Bs,0:As,0:Ys),ETA_M(Bs,0:As,0:Ys),ETA_TERMINAL(Bs,0:As),ETA_TAG_SURV(Bs,0:As,0:Ys), &
                         ETA_TAG_LOSS(Bs,0:As,0:Ys),ETA_TAG_REPORT(Bs,0:As,0:Ys),TAG_NOMIX(Bs,0:As,0:Ys),ETA_TAG_NOMIX(Bs,0:As,0:Ys), &
                         ETA_SCALE_VARIANCE(Gs,Bs),TAG_NOMIX2(Bs,0:As,0:Ys),ETA_TAG_NOMIX2(Bs,0:As,0:Ys)
        INTEGER :: FIRSTYEAR, LASTYEAR, DISPLAYYEAR, NYEARS, FIRSTAGE, LASTAGE, YEAR, AGE, GEAR, BOX, &
                   PLUSGROUP,NGEARS(Bs),SEASON_EFFORT(Gs,Bs),SEASON_SSB(Bs),AGE_EFFORT(2,Gs,Bs), &
                   STARTYEAR(Bs),ENDYEAR(Bs)
      ...
      MODULE LOGLIKELIHOODS
        ...
        REAL (KIND=8) :: LIKE_RECRUITMENT(Bs),LIKE_EFFORT(Gs,Bs),LIKE_TAG(Bs),LIKE_M(Bs),LIKE_T(Bs), &
                         ...
                         LIKE_TAG_REPORT(Bs),LIKE_TAG_SURV(Bs),LIKE_TAG_LOSS(Bs),LIKE_TAG_NOMIX(Bs),LIKE_V(Bs), &
                         SUM_EFFORT_DISCREPANCY(Gs,Bs),EFFORT_DISCREPANCY(Gs,Bs,0:Ys),DISCREPANCY
...
!FLR      WRITE(*,'(/,/,/,/,/,/,/,/,/,/,/,/,/,/,/,/,/,/,/,/,/,/)')
!FLR      WRITE(*,*) '+-------------------------------------------------------------------------+'
!FLR      WRITE(*,*) '| Program VPA-2BOX.F90, Version 3.01 (March 17, 2003)                     |'
!FLR      WRITE(*,*) '|                                                                         |'
!FLR      WRITE(*,*) '| A virtual population analysis tool that uses catch-at-age, indices of   |'
!FLR      WRITE(*,*) '| abundance, indices of mortality rates, and tag-recoveries to estimate   |'
!FLR      WRITE(*,*) '| the abundance and mortality of one or two (intermixing) populations.    |'
!FLR      WRITE(*,*) '|                                                                         |'
!FLR      WRITE(*,*) '| based on the methods of                                                 |'
!FLR      WRITE(*,*) '|   Porch, C. E., Turner, S. C., and Powers, J. E. 2001                   |'
!FLR      WRITE(*,*) '|   Virtual population analyses of Atlantic bluefin tuna with alternative |'
!FLR      WRITE(*,*) '|   models of transatlantic migration: 1970-1997. Int. Comm. Conserv. Atl.|'
!FLR      WRITE(*,*) '|   Tunas, Coll. Vol. Sci. Pap. 52: 1022-1045                             |'
!FLR      WRITE(*,*) '|                                                                         |'
!FLR      WRITE(*,*) '| programmed by                                                           |'
!FLR      WRITE(*,*) '|   Clay E. Porch                                                         |'
!FLR      WRITE(*,*) '|   NOAA Fisheries                                                        |'
!FLR      WRITE(*,*) '|   75 Virginia Beach Drive                                               |'
!FLR      WRITE(*,*) '|   Miami, Fl 33149 (USA)                                                 |'
!FLR      WRITE(*,*) '+-------------------------------------------------------------------------+'
!FLR      WRITE(*,'(/,/)')
...
!_________________________________________________________________      
      USE PARAMETERS ; USE DATUM, ONLY : PDF_STOCKRECRUIT, IGNORE_RECRUIT
      IMPLICIT NONE
      INTEGER :: IDUMMY,I,LINE=0
      REAL (KIND=8) :: DUMMY
      CHARACTER (LEN=1) :: CH,CH2,CH3
!FLR     CALL GETCL(CONFILE)
      CONFILE="C:\Projects\FLR\Assessment Methods\VPA2Box\Data\alb00.c01"
...
2       READ(10,'(3A1)')  CH,CH2,CH3 ; LINE=LINE+1
      ...
      IF(ABS(OPTION(3))>0 .AND. N_RETRO>0) THEN
        WRITE(*,*) 'ERROR: You cannot do retrospective analyses and bootstraps at the same time' ; STOP
      ENDIF
      RETURN
10    WRITE(*,'(1X,A24,A50)') 'ERROR: The control file ',CONFILE
      WRITE(*,'(8X,A55)') 'does not exist or is being used by another application.'
      WRITE(*,'(1X,A50)') 'Use CONTROL-C to abort or enter a new file name:  ' ; READ(*,*) CONFILE
...
!       wade through comments and read the parameter specifications
        IF(CH.EQ.'@') THEN
          ...
          ELSEIF(CH.NE.'#'.AND.CH.NE.'!'.AND.CH.NE.'_') THEN
            IF(CH2=='#'.OR.CH2=='!'.OR.CH2=='@'.OR.CH3=='#'.OR.CH3=='!'.OR.CH3=='@') THEN ;
              WRITE(*,'(1x,a23,a1,a4,a1,a8)') 'INPUT ERROR: Misplaced ',CH2,'and ',CH3,' symbols'
              WRITE(*,'(1x,a21,i3,a8,a50)')   '             in line ',line,'of file ',INFILE(2) ; STOP
            ENDIF
            BACKSPACE(11)
...
!             identify improperly-specified boundary conditions of free parameters
              IF(DEFAULTS(1).GT.DEFAULTS(2)) THEN
                   ...
                   WRITE(*,'(1x,a25,i4,a8,a50)')   '       specified on line ',line,'of file ',INFILE(2) ; STOP
...
!----------------------------------------------------------------------------
      ...
      IF(PARM_SPECS(I,4)>0.01 .AND. PARM_SPECS(I,4)<0.99) THEN ! Bayesian structures
          ...
          IF(PARM_SPECS(I,4)<0.11) THEN ! Correlated random deviation structure (random walk)
              ETA=LOG(VARIABLE/OLD_VARIABLE)    ; STAR_VARIABLE=VARIABLE ; OLD_VARIABLE=VARIABLE
            ELSEIF(PARM_SPECS(I,4)<0.21) THEN ! Uncorrelated random deviation structure
              ETA=LOG(VARIABLE/STAR_VARIABLE)   ; OLD_VARIABLE=VARIABLE
            ELSEIF(PARM_SPECS(I,4)<0.31) THEN ! random deviations from prior expectation of present parameter
              ETA=LOG(VARIABLE/PARM_SPECS(I,2)) ; STAR_VARIABLE=VARIABLE ; OLD_VARIABLE=VARIABLE
          ENDIF
...
!------------------------------------------------------------------------------
      ...
      IF(PARM_KEY(0).GT.NPAR) THEN
        ...
        WRITE(*,*) '        EITHER REFORMULATE THE MODEL OR INCREASE THE DIMENSIONS IN THE DUMMY ARRAYS' ; STOP
      ENDIF
      ITER=MAXITER
...
!FLR      WRITE(*,*) 'WRITING TO FILE ',INFILE(8)
      ...
      SELECT CASE (CHOICE)
        ...
        CASE (1) ! lognormal
          IF(OBS.EQ.0) THEN
            ...
            WRITE(*,*) '        observations is zero-- LOG(0) is undefined' ; PAUSE ; STOP
          ENDIF
...
!             {negative predictions are infeasible}
        ...
        CASE (7) ! Gamma (using Stirling's approximation for the gamma function)
          IF(OBS.EQ.0) THEN
            ...
            WRITE(*,*) '        observations are zero-- LOG(0) is undefined' ; STOP
          ENDIF
          IF(SIGMA>0) THEN  ! {convert cv's to std. dev.'s}
              IF(PRED>0) THEN
                  SIGMA = ABS(SIGMA*PRED)
...
!--------------------------------------------------------------------------
      ...
      SELECT CASE (ICHOICE)
        ...
        CASE (1) ! lognormal
          IF(XBAR.LE.0) THEN
            WRITE(*,*) ' ERROR: The lognormal density was selected for the likelihood, but one of the'
            WRITE(*,*) '        observations is zero-- LOG(0) is undefined' ; STOP
          ENDIF
        ...
        CASE (3) ! multinomial
          WRITE(*,*) 'MULTINOMIAL DENSITY NOT AVAILABLE FOR RANDOM VARIABLE GENERATION' ; STOP

        CASE (4) ! Poisson
          IF(XBAR<0) THEN
              WRITE(*,*) 'ERROR: The Poisson density was selected, but the input expectation is < 0' ; RETURN
            ELSE
              a=EXP(-XBAR) ; b=1 ; ib=0
              do 20 i=1,1000
                b=b*RAN1(iseed)
...
20            CONTINUE
        ...
        CASE (5) ! chi-square
          WRITE(*,*) 'CHI-SQUARE DENSITY NOT AVAILABLE FOR RANDOM VARIABLE GENERATION' ; STOP

        CASE (6) ! double-exponential
          WRITE(*,*) 'DOUBLE EXPONENTIAL DENSITY NOT AVAILABLE FOR RANDOM VARIABLE GENERATION' ; STOP
...
1     READ(10,'(3A1)')  CH,CH2,CH3 ; LINE=LINE+1
        IF(CH.NE.'#'.AND.CH.NE.'!'.AND.CH.NE.'_'.AND.CH.NE.'@') THEN
            IF(CH2=='#'.OR.CH2=='!'.OR.CH2=='@'.OR.CH3=='#'.OR.CH3=='!'.OR.CH3=='@') THEN ;
              WRITE(*,'(1x,a23,a1,a4,a1,a8)') 'INPUT ERROR: Misplaced ',CH2,'and ',CH3,' symbols'
              WRITE(*,'(1x,a21,i3,a8,a20)')   '             in line ',line,'of the control file.' ; STOP
            ENDIF
            BACKSPACE(10)
          ...
          ELSEIF(CH.NE.'@') THEN
            GOTO 1
          ELSE
            WRITE(*,*) 'INPUT ERROR: improperly placed @' ; STOP
        ENDIF
      END
...
!_________________________________________________________________      
      ...
      IF(I.GT.IDIM) THEN
       ...
       ELSE
        GOTO 1
      ENDIF
      WRITE(*,'(1X,A9,I5)') 'Record # ',RECORD ; STOP
1     RETURN ; END
...
! {Update the results report every 5 iterations}     
      IF(DO_PRINT.EQ.1 .AND. MOD(NTRY,5).EQ.0) CALL RESULTS(THE_END)      ! A user-defined subroutine
...
! check for convergence by comparing the results of the last 5 iterations
      ...
      REAL (KIND=8) :: parmest(NP+1),parmold(5,NPAR),r
      IF(NFLAG.GT.5) NFLAG=5
      GOOD=0
...
!--------------------------------------------------------------------------------
      ...
      IF(NV==1) THEN  ! use bisection
          ...
          XINP=XVEC(1) ; XVEC(1)=0.0 ; CALL FUNC(NV,XVEC,FVEC,DF,I)
          IF(FVEC(1)<0) THEN
