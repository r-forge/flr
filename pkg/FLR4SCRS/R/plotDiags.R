...
setMethod("plotDiags", signature(object="data.frame"),
  function(object,col="#CCFF33",group=1,stuff=facet_wrap(~age,scale="free"),plots=c(1,2,3,4,5,6)){

  p<-ggplot(data.frame(x,col=col,group=group))
  ...
  grid.newpage()
  if (length(plots)>1)
     pushViewport(viewport(layout=grid.layout(floor(length(plots)/2)+1,2))) else
