...
# PURPOSE: compare estimated fishing mortality from ICA and the simulated data
# DATE: 9 September 2005
# LAST MODIFIED: 21 September 2010
# AUTHOR: marco.kienzle@gmail.com

# STATUS not working because not finished
...
print R "f.simulated <- read.table(paste(\"$DataPath/\",number,\"/simulated.fishing.mortality\",sep=\"\"))\n";

print R "f.estimated.by.ica <- read.table(paste(\"$DataPath/\",number,\"/ica.f\",sep=\"\"), skip=5, colClasses=\"numeric\")\n";

#print R "estimated.by.w <- read.table(paste(\"$DataPath/\",number,\"/ica.f\",sep=\"\"), skip=5, colClasses=\"numeric\")\n";

#print R "par(mfrow=c(2,2))\n";
#print R "plot(as.matrix(f.simulated[1,1:9]), as.matrix(estimated.by.w[1,1:9]), xlab=\"simulated\", ylab=\"version w\");for(i in 2:9){points(as.matrix(simulated[i,1:9]), as.matrix(estimated.by.w[i,1:9]))}\n";
#print R "abline(0,1)\n";

print R "plot(as.matrix(f.simulated[1,1:9]), as.matrix(f.estimated.by.ica[1,1:9]),xlab=\"simulated\", ylab=\"estimated\", main = \"ICA NAG 22 against simulated fishing mortalities\");for(i in 2:9){points(as.matrix(simulated[i,1:9]), as.matrix(estimated.by.x[i,1:9]))}\n";
print R "abline(0,1)\n";
...
# print R "old<-read.ica(paste(\"$DataPath/\",number,\"/ica.out\",sep=\"\"))\n";
# print R "new<-read.ica(paste(\"$NewVersionPath/\",number,\"/ica.out\",sep=\"\"))\n";

# print R "simulated <- read.table(paste(\"$DataPath/\",number,\"/simulated.ssb\",sep=\"\"), skip=2, header=TRUE)\n";
