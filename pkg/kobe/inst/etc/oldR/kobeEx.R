...
assmt=subset(sims, Run=="run02" & year<=2010 & TAC==15000)
...
trks=ddply(tmp,.(Run,TAC,year), function(x) kobeTrks(x[,c("stock","harvest")],prob=c(0.25,0.5,0.75)))
...
sim2x=function(x){
  ...
  x=ddply(x, .(variable), function(x) k2smFn2s(x[,-3]))
  x}
