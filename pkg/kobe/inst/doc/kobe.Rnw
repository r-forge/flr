...
## plots ##############################################################################################################
## summary of data
fig1=ggplot(subset(kobe.ts, year %in% 2005:2020)) + geom_boxplot(aes(factor(year),ssb)) +
                               facet_wrap(~TAC,ncol=2) +
                               scale_x_discrete(breaks=seq(2005,2020,5)) +
                               xlab("Year")+ylab(expression(SSB/B[MSY])) +
                               geom_hline(aes(yintercept=1),col="red")
                               
fig1+geom_line(aes(year,ssb,group=iter,col=iter),data=subset(kobe.ts, year %in% 2005:2020 & iter %in% sample(1:100,5)))
  
                                                      
                                  
fig2=ggplot(subset(kobe.ts, year %in% 2005:2020)) + geom_boxplot(aes(factor(year),harvest)) +
                               facet_wrap(~TAC,ncol=2) +
                               scale_x_discrete(breaks=seq(2005,2020,5)) +
...
.fig2=ggplot(smry.ts)+geom_line(aes(year,ssb,  colour=factor(TAC),group=TAC),size=2) +
                               xlab("Year")+ylab(expression(SSB/B[MSY])) +
                               geom_hline(aes(yintercept=1),col="red")

.fig3=ggplot(smry.ts)+geom_line(aes(year,harvest,colour=factor(TAC),group=TAC),size=2) +
                               xlab("Year")+ylab(expression(F/F[MSY])) +
                               geom_hline(aes(yintercept=1),col="red") +
                               scale_y_continuous(limits=c(0,2))
...
## pie charts
fig7=ggplot(subset(pie.dat,year %in% seq(2011,2021,3) & TAC %in% c(3000,4000,5000)), aes(x ="", y=value, fill = factor(variable))) + 
        geom_bar(width = 1) + 
        coord_polar(theta = "y") +
...
k2sm(subset(kobe.ts,year>=2010)[,c("year","TAC","green")])
@
\caption{K2SM}
\end{center}
\end{figure}
