...
io=function(x,prob=c(0.75,0.5,.025),yrs=NULL,pts=NULL,nwrms=10,what=c("ts","smry","pts","wrms"),proxy="msy"){
    ...
    res=as.data.frame(FLQuants(stock  =stock.obs(  x)%/% refpts(x)["bmsy"],
                               harvest=harvest.obs(x)%/% refpts(x)["fmsy"]))
    ...
    if ("ts" %in% what){ 
      ...
      ts=data.frame(melt(stock,id.vars=c("year","tac")),
...
setMethod('kobeDat', signature(object='FLBioDym'),
