...
kobe(subset(smry.ts,year<=2010))+geom_path(aes(ssb,harvest,colour=TAC,group=TAC))+
...
<<results=hide, echo=TRUE>>=
kobe(subset(smry.ts,year>=2010))+geom_path(aes(ssb,harvest,colour=factor(TAC),group=TAC),size=2)
@
...
Pie charts can also be produced, for example if a quota of 4000 is chosen then the proportion of time spent in
the different quadrants can be shown.
...
## kobe plot
kPlot=kobe() + 
         geom_point(aes(ssb,harvest),colour="blue",size=1,data=subset(kobe.ts,year==2010)) + 
         geom_path( aes(ssb,harvest),colour="blue",data=subset(smry.ts,year<=2010))  
        
# First density plot
p1<-ggplot(subset(kobe.ts,year==2010)) + 
    #scale_fill_manual(values=cbnbPalette) + 
    geom_density(aes(x = ssb, y =  ..count..), fill="#CCCCCCCC", position = "stack") + 
    geom_density(aes(x = ssb, y = -..count.., alpha=0.4)) + 
    theme_invisible() +
...
p2<-ggplot(subset(kobe.ts,year==2010)) + 
    #scale_fill_manual(values=cbnbPalette) + 
    geom_density(aes(x = harvest, y =  ..count..), fill="#CCCCCCCC", position = "stack") + 
    geom_density(aes(x = harvest, y = -..count..),alpha=0.4) + 
    theme_invisible() + 
    opts(legend.position = "none") +coord_flip()+scale_x_continuous(limits=c(0,2))+
    geom_vline(xintercept=1,col="red")
...
# First density plot
k1=kobe()
df=subset(kobe.ts,year==2010 & TAC==0)


k2=k1+geom_point(aes(x,y,alpha=freq/max(freq)),data=calcFreq(df$ssb,df$harvest,x.n=7),size=1.2)


#k3=k2 + stat_contour(aes(x = x, y = y, z = z),data=calcDensity(df$ssb,df$harvest,x.n=50), breaks=c(1,5),col="cyan")


#k4=k2 + geom_path(aes(x,y,group=level),colour="blue",data=calcProb(df$ssb,df$harvest,prob=c(0.6,0.9))) + 
#     theme_complete_bw() + 
