...
\description{
  ...
  although the plot method is specific to FLBioDym, and methods exist to access the specific
...
\item{ signature(model=character) :}{Creates an \code{FLBioDym} from an already existing stock-recruitment model
...
\item{ signature(model=function) :}{Creates an \code{FLBioDym} object using information given in the function.
    This function can be an empty function in which case an empty \code{FLBioDym} will be returned or a
...
\examples{
    # Create an empty FLBioDym object.
    sr1 <- FLBioDym()
    
    # Create an  FLBioDym object using the existing SR models.
    sr2 <- FLBioDym(model = 'ricker')
    sr2@model
    sr2@initial
    sr2@logl
    
    sr3 <- FLBioDym(model = 'bevholt')
    sr3@model
    sr3@initial
    sr3@logl
    
    # Create an FLBioDym using a function.
    mysr1 <- function(){
        model <- rec ~ a*ssb^b
        return(list(model = model))}
    
    sr4 <- FLBioDym(model = mysr1)

    # Create an FLBioDym using a function and check that it works.
    mysr2 <- function(){
        formula <- rec ~ a+ssb*b
    ...
    sr5 <- FLBioDym(model = mysr2, ssb = ssb, rec = rec)
