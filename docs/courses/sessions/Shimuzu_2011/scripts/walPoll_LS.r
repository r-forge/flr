...
wpData<-read.csv('/home/rob/Dropbox/FLRbook/Courses/Shimuzu/Server/Data/WSData/WalleyePollock.csv')

source('/home/rob/Dropbox/FLRbook/Courses/Shimuzu/scripts/laurecshepherd.r')
...
# age based cpue 
catch.prop <- sweep(catch.n(wallPol)[,ac(1998:2008)], 2, apply(catch.n(wallPol)[,ac(1998:2008)],2,sum), '/')
cpue.age <- FLIndex(sweep(catch.prop, 2, cpue, '*'))
catch.n(cpue.age) <- sweep(catch.prop, 2, cpue, '*')
effort(cpue.age)[]   <- 1
...
# Run Laurec Shepherd
lsvpa <- lsm(wallPol, cpue.age, fratio=1, fit.plusgroup=T)

pfun <- function(x,y,...){
  panel.xyplot(x,y, cex=0.7, ...)
  panel.loess(x,y, ...)
  panel.abline(h=0, lty=2, col='grey')
}
xyplot(data~year|age, data=index.res(lsvpa), type=c('g','p'), panel=pfun, ylim=c(-2,2))
...
xyplot(data~age, group=year, type="l", xlab="Age", ylab="Selection",
       data=FLQuant(sweep(harvest(wPol),2,apply(harvest(wPol),2,max),"/")))

#Time series of fishing mortality
xyplot(data~year, data=harvest(wPol)[ctrl@sep.age,ac(1990:2008)], type="l",xlab="Year", ylab="Fishing Mortality", ylim=c(0,1.25))

#Bubble plot of catch residuals
catchHat <- harvest(wPol)/Z(wPol)*stock.n(wPol)*exp(-Z(wPol))
pfun <- function(x,y,...) {
  panel.xyplot(x,y,col='black',...)
  panel.loess(x,y,col='black',...)
  panel.abline(h=0, lty=2, col="grey")
}
xyplot(data~year|ac(age), data=log(catch.n(wPol)/catchHat), type=c('g','p'), 
       panel=pfun, ylab='log(C/predC)', xlab='Year')
