...
The Generic Life History Generator is a collection of tools for generating a fully specified, biologically plausible, age-structured stock object based on basic biological parameters and a fishing selection pattern.
...
The default growth model is a combination of the von Bertalanffy growth equation (that gives length at age) and a length-weight relationship. The lengths-at-age are given by:
\begin{equation}
...
The selection pattern determines the relative fishing mortality at age. This is specfied as something awesome.
The default selection pattern is based on a double normal (Casal reference)
...
Where $sel$ is the selction pattern at age, $x$ is the age or length, $a$, $sL$, $sR$ are something else.

\subsection{Natural mortality}

By default, the natural mortality is calculated using the function from ~\cite{Gislason2008514}:

\begin{equation}
%M     <-function(L,Linf,M1=0.1,h=1.71,n=-1.66,i=0.8) M1+h*Linf^i*L^n
M = M_1 + h L_{\infty}^i L^n
\end{equation}

Where $M$ is the natural mortality at age, $M_1$ is something useful, $h$ is amazing, $L_{\infty}$ is the asymptotic length (also used in the von Bertalanffy equation to calculate growth), $i$ is whatever and $n$ is made up.

\subsection{Maturity}

The default maturity equation is:
...
%     }
% 
if ((Mat_{50} - x) / Mat_{95}) > 5 & \nonumber \\
& Mat = 0 \nonumber \\
else if ((Mat_{50} - x) / Mat_{95}) < -5 & \nonumber \\
& Mat = 1 \nonumber \\
else & \nonumber \\
& Mat = \frac{1}{1 + 19^{(Mat_{50}-x)/Mat_{95}}}
%& Mat = 1 \ (1 + 19^{(Mat_{50}-x)/Mat_{95}})
\end{eqnarray}
...
\end{equation}
Where $Mat_{50}$ is the age at 50\% maturity, $M$ is the natural mortality at age (given by the natural mortality relationship described above) and $k$ is the rate from the growth equation (see above).

\subsection{Stock recruitment relationship}

The stock recruitment relationship is specified as a list of parameters and model name or an \texttt{FLSR} object.
...
Default values are supplied for $t_0$, $a$, $b$, $M_1$, $h$, $n$ and $i$. This means that only $k$, $L_{\infty}$, $M_{95}$ and the selectivity parameters need to be specified to completely determine the stock.

When combined with the stock recruitment relationship it is possible to calculate the stock status at equilibrium under a range of fishing intensities. This is done using \texttt{FLBRP}.

\section{Demonstration of stock creation}

test bib ~\cite{Nobody06}.
References - Gislason, Casal

\section{Testing assessment methods}

\bibliography{FLHbib.bib}{}
