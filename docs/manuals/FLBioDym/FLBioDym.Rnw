...
The productivity (Pt) of a stock can be defined as the surplus production, i.e. following Hilborn (2001):
...
\subsection{Schaefer}{Schaefer}\index{Schaefer model}
...
\subsection{Theta-logistic}{Theta-logistic}
...
\subsection{Polacheck}{Polacheck}
