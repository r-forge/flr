...
ggplot(pbft)+geom_histogram(aes(variable,weight=value))+faccet_wrap(~year)
...
## catch proportions
sweep(caaPol,2,apply(caaPol,2,mean),"/")

## plot as bubbles
p<-ggplot(sweep(caaPol,2,apply(caaPol,2,mean),"/")) +
      geom_point(aes(year,age,size=data))
      
p<-p + scale_area(to = c(0, 15))
p
p<-p +opts(title="Selectivity-at-age")
p